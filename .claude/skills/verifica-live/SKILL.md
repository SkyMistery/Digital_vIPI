...
| `testo-verifica.js` | la **barra di formattazione** dei campi di prosa: i sette tasti come interruttori, gli elenchi su piu' righe, gli **annidati** (Tab/Maiusc+Tab veri, ⇤ ⇥, Invio che continua, Tab su un capoverso che cambia campo), Ctrl+B/I/U, e che quel che i tasti scrivono — un secondo livello fatto col Tab compreso — **sopravviva a un ricarico** e si renda annidato. `node testo-verifica.js http://localhost:5199/services/vsop/libb/editor` | quando si tocca `RichTextArea`, `MarkdownLite`, `VoceDiElenco` o `vipiMdWrap`/`vipiMdList`/`vipiMdRientro` |
| `aor-famiglie-verifica.js` | vIPI ACC: le sezioni «Settori militari» e «Settori FSS» stanno **prima e dopo** le aree regolamentate, e l'AoR in cima **non** ha chip `*_MIL_*`/`*_FSS` (21 settembre 2026). Stampa i chip di ogni mappa. Sola lettura. ⚠️ In locale la vIPI non è pubblicata: passare `…/vipi?as=draft`; `EDITOR=1` per l'editor, `ACC=LIBB` per un altro ACC | quando si tocca `AccFamigliaAorRegola`, `DeriveAorViewAsync` o il catalogo `AccAerovia` |
| `procedura-nei-punti-verifica.js` | Trasferimenti: fra i suggerimenti dei punti ci sono le SID/STAR dello scalo della sezione; scelta una procedura, il luogo diventa `AorBoundary`, «come l'ingresso» si spegne e l'anteprima dice «autorizzato via … al confine dell'AoR» (21 settembre 2026). Non salva. ⚠️ In locale le STAR **non ci sono** (arrivano dal sectorfile, spento): provare le SID con `PARTENZA=1` su `?acc=LIBB&accordo=4&sezione=8`; `SCALO=` per un altro scalo. Il catalogo punti in locale è vuoto: i soli suggerimenti sono le procedure | quando si tocca `ProceduraNeiPunti`, `PointItems`/`LoadProceduresAsync` o il form della clausola |
| `cita-cursore-verifica.js` | «Cita» scrive **dove sta il cursore**: col fuoco nel campo, **senza** fuoco (un clic fuori prima del tasto — era il difetto del 21 settembre 2026, il riferimento finiva in coda) e nel campo **mai toccato** (lì va in coda, per scelta). Tre lanci, uno per caso: `PERDI_FUOCO=1`, `MAI_TOCCATO=1`. Solo in locale: scrive nel documento | quando si tocca `vipiSidPrendi`/`vipiSidInserisci`, `RichTextArea` o `TastoRiferimentoTabella` |
| `aree-mil-verifica.js` | le **aree di lavoro del vSOP militare** (§A40): una tabella sola e nessuna scheda, la freccetta ▸ che apre il dettaglio con `aria-expanded`, la chip che spegne riga **e** dettaglio e aggiorna il conteggio, riaccesa chiusa, in stampa dettagli aperti e freccette via; e la vIPI ACC che **tiene** le schede | quando si tocca `MilWorkingAreas`, `RegulatedAreas` (`ShowCards`), `setCard` in `vipi-aor.js` o `wireRigheArea` in `vipi-ui.js` |
...
| `enhanced-verifica.js` | la **navigazione enhanced**: dall'elenco vAWOS a uno scalo i tasti commutano con UN clic (T-016), e le mappe AoR restano disegnate cambiando «Tutto/ATC/Pilota» (T-015; `DOC=/services/vsop/libb/mil?icao=LIBG` sul DB di sviluppo, `DOC=/services/vsop/lirr/mil?icao=LIBA` in produzione — LIBG lì non ha un vSOP pubblicato). ⚠️ Controlla che le mappe fossero disegnate PRIMA del clic, o «vuote dopo» non prova niente | quando si tocca un segno «già agganciato» in un modulo JS (`vipi-aor*.js`, `vipi-mva.js`, `vipi-awos.js`, `vipi-ui.js`) |
| `mappa-foglio-verifica.js` | il **foglio di Leaflet** dopo la navigazione enhanced (§A99, 21 settembre 2026): pagina senza mappa → documento → pagina senza mappa → documento, tutto con `Blazor.navigateTo`; al secondo arrivo il `leaflet.css` deve esserci e le tessere essere `position:absolute`. Sul codice di prima: 0 fogli, 126/126 tessere fuori posto. `DOC=/services/vsop/libb/editor` per l'editor — ⚠️ lanciarlo da **PowerShell**: Git Bash traduce `DOC=/services/…` in `C:/Program Files/Git/services/…` | quando si tocca il caricamento di Leaflet in `vipi-aor.js`, `vipi-boot.js` o il `<head>` di `App.razor` |
| `star-nascoste-verifica.js` | le sezioni **STAR** degli aeroporti nascono **nascoste** (§A100): in bozza la testata STAR porta «🚫 nascosta (non pubblica)», la SID no, e l'editor elenca la STAR. `ACC=libb ICAO=LIBD`. ⚠️ Le chip «SID/STAR» senza segno non sono testate | quando si tocca `BornHidden` nel catalogo, `DocumentBirth` o `AddMissingCatalogSectionsAsync` |
| `porte-verifica.js` | le **porte** delle pagine: gesti a raffica su Ricerca, Audit, Glossario e Versioni senza circuito caduto né errori in console (lotto L5 del 13 settembre 2026) | quando si tocca `ScopeProprioCheAspetta`, una sentinella di rientro o una di quelle quattro pagine |
...
| `pannelli-pubblicato-verifica.js` | **elenco aeroporti** e **vista rapida** (live) decidono la pista consigliata sul documento PUBBLICATO (`IPisteDalPubblicato`, come il vAWOS). Il trucco che non dipende dal vento vero: nell'anagrafica VIVA escludere tutte le soglie dello scalo in tutti e due i versi — dai vivi uscirebbe «—» — e controllare che i pannelli mostrino una pista. Avviare con `Ivao__FakeOnlineCallsigns=LIBD_TWR` per la vista rapida | quando si tocca `IPisteDalPubblicato`, `AirportListPanel` o `AirportQuickPanel` |
| `adatta-verifica.js` | il **campo che si adatta al testo** (`data-adatta`): in un editor vero, un campo di prosa deve crescere quando il testo cresce. Prova **doppia** — la stessa misura si rifà rimettendo a mano `.app-block-edit .app-ta{flex:1}`, che è la regola che l'aveva spento: se cresce anche con quella, la prova non distingue e il guasto è altrove. `node adatta-verifica.js "<editor>?icao=LIBD"` | quando si tocca `vipiAdatta` in `vipi-editor.js`, `RichTextArea`, o una regola di `.rta`/`.app-block-edit`/`.app-ta` nel foglio |
| `sid-verifica.js` | i **riferimenti citati nel testo** (§A73 SID, §A80 STAR): il tasto «Cita» di un campo di prosa e quello sotto una tabella — cursore segnato al clic con un clic VERO (`ElementHandle.click`, per provare il `preventDefault`), fuoco nella ricerca del selettore, **la chip STAR che ricarica l'elenco**, riferimento scritto dove stava il cursore, selettore che si chiude, e che tutto **sopravviva a un ricarico**; poi l'anteprima dell'editor col nome completo e mai `[[SID`/`[[STAR`. ⚠️ Scrive nel documento (rifiuta `ivao.aero`). `node sid-verifica.js "<editor>?icao=LIBD" BANAV` | quando si tocca `RichTextArea`, `RiferimentoPicker`, `TastoRiferimentoTabella`, `vipiSidPrendi`/`vipiSidInserisci` o le anteprime di `DocumentSectionsEditor` |
