...
| `sid-verifica.js` | le **SID citate nel testo** (§A73): il tasto «SID» di un campo di prosa e quello sotto una tabella — cursore segnato al clic con un clic VERO (`ElementHandle.click`, per provare il `preventDefault`), fuoco nella ricerca del selettore, riferimento scritto dove stava il cursore, selettore che si chiude, e che tutto **sopravviva a un ricarico**; poi l'anteprima dell'editor col nome completo e mai `[[SID`. ⚠️ Scrive nel documento (rifiuta `ivao.aero`). `node sid-verifica.js "<editor>?icao=LIBD" BANAV` | quando si tocca `RichTextArea`, `SidPicker`, `TastoSidTabella`, `vipiSidPrendi`/`vipiSidInserisci` o le anteprime di `DocumentSectionsEditor` |
