# Pacchetto 1.46.3 — solo i file cambiati

> **Timbro:** `1.46.3 · de5af3a` (25 settembre 2026). Lo vedono gli amministratori nella barra in alto. Se la
> barra è stretta, sta nella riga `Versione` della **Diagnostica** e in quella di
> `diagnostica/avvio-diagnostica.txt`.

> **Parte da 1.46.2** (`f30c036`, online dal 25 settembre). È una **PATCH, NESSUNA migrazione**: si consegna da
...
- **Pagina Trasferimenti: il pannello di una clausola non si apre più con gli elenchi vuoti.** Il 25 settembre, su
  LICC, aprire una clausola leggeva piste e SID/STAR mentre la pagina stava ancora leggendo altro dal database, e
  il pannello si apriva senza piste né punti, senza dirlo. Ora le letture della pagina si fanno **una per volta**.
...
`Vipi.Host.staticwebassets.endpoints.json` e tutto `wwwroot`: identici a 1.46.2, controllato per impronta.
...
- il timbro **`1.46.3 · de5af3a`**, e in `services/vsop/admin/diagnostics` la riga **`Schema` = `0`**;
- facoltativo: `services/vsop/admin/transfers`, un accordo con un aeroporto (per esempio LICC), «Modifica», aprire
  una riga nel pannello → sotto «Condizione» devono esserci le **piste** dell'aeroporto.
- La prova vera è nel prossimo scarico di diagnostica: in `avvisi-log.txt` non devono più comparire «Trasferimenti:
  piste di … non lette» né «A second operation was started» dalla pagina Trasferimenti.
