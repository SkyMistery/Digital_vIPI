...
**Ultimo aggiornamento:** 17 settembre 2026 (notte) — ✅ **1.32.0 È ONLINE** (§A68: timbro e `Schema 0` confermati, colonna «Mai usare» presente in vIPI e vSOP). 🟡 **§A69 in main, NON in pacchetto**: le caselle DEP/ARR diventano chip `sh-chip` come le colonne accanto, **rosse** quando accese (`sh-chip.no`); solo `Vipi.Ui` + `vipi-theme.css`, nessuna migrazione. Test bUnit provati per mutazione, verifica dal vivo in scuro/chiaro e sull'editor militare, contrasto 5,56:1. ▶ **Va col prossimo pacchetto** (decisione del committente): niente PATCH a parte. ▶ **Si riparte da §A70 di `docs/lavori-aperti.md`** (ripubblicare LIBA_APP e gli APP agganciati pubblicati sotto 1.31.0; aperti ereditati da §A64). 🔴 net10 = **1.33.0**.
