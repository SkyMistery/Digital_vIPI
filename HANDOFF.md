...
- **Slice 2, la sessione**: `CartellaDelSector` (risale fino alla radice del clone), `Confini` (si scrive solo in `SectorFiles/Include/IT/**`, `SectorFiles/update.ini`, `changelog.md`), `SessioneAperta` (albero intero in parallelo, **impronta SHA-256 per file** presa prima e ricontrollata dopo la lettura, `CambiatiSulDisco()`, `.tmp` orfani). Albero vero: 701 file letti, 118 315 record, **0,35-0,48 s** (1,3 s la prima), 102 MB.
- **Slice 3, catalogo e geometria**: `CarichiDegliIsc` e `Cataloghi.Dichiarato` **estratti dal validatore nel motore** (li usano validatore e app: stessi numeri, 130 errori / 353 avvisi); `CatalogoDeiPunti.PerOgniIsc` (5 master in 39 ms, ITALY 4 465 punti); `Geometria.DelFile` (**21 667 forme, 244 227 punti, 42 ms**). 🔴 Corretto un difetto del motore: i `.rw` di FIR scrivono `///////PISTE` e **187 righe di pista** erano invisibili al modello (non è un errore del sector: non va agli AOD).
- **Slice 4, la mappa**: Leaflet canvas servito da noi, **nessuno sfondo a tessere**; **13 strati** decisi nel `Core` (`StratiDellaMappa`: `GEO/itgeo.geo` è lo sfondo di tutti, gli altri `.geo` no); la geometria non passa dal circuito ma da una `fetch` a `/mappa/strato/<id>`, dietro lo stesso cancello (403 senza cookie), in JSON corto a cinque decimali; clic su una forma = la sceglie, col riquadro che dice file, record, punti e **i nomi che il master non risolve**. Sull'albero vero: **21 854 forme disegnate in 436 ms**, app pronta in 2,7 s, JSON 5,8 MB in tutto. Il guscio ha ora `--cartella <percorso>` e `--tutti-gli-strati` (con `--autoprova` la prova aspetta «mappa disegnata»), e i tempi di ogni strato vanno nel diario d'avvio. 🔴 **Un componente Blazor senza parametri non viene ridisegnato quando cambia il padre**: `Mappa.razor` si iscrive da sé a `SessioneDelLab.Cambiata` — trovato da tre test bUnit, non dallo schermo.
- **Slice 5, sfoglia e ispettore** (in lettura): l'albero delle cartelle costruito **dai file già aperti**; la **ricerca per nome** fra le forme della mappa (esatto → comincia → contiene; ⚠️ i record senza geometria, `.frq` e ATIS, non ci sono ancora: ci si arriva sfogliando); l'**ispettore** coi campi letti per **riflessione** sul modello del motore (una tabella per tipo sarebbe una seconda verità che invecchia in silenzio) e le **righe grezze col numero vero del file**, quello che cita il validatore, con tre righe di contesto. Scegliere una forma sulla mappa apre il suo file nell'albero. Albero in 6 ms, ricerca 0-11 ms, etichette del file più grosso (13 560 record) in 6 ms. 🔴 Due etichette sbagliate trovate dalla **misura** e non dai test: 201 «AtcPosition» (i `.frq` si chiamano col loro `Code`) e 13 560 «Line» (i segmenti `.geo` col colore, `COAST`). 🔴 Una **prova che non distingueva** (i nomi del test erano in ordine alfabetico *e* di punteggio). 🔴 `bUnit` 1.40 con AngleSharp 1.6: `FindAll(...)[0]` lancia `MissingMethodException`, si usa `.First()`.
- **Slice 6, modifica dei campi** (in memoria: sul disco si scrive dalla slice 9) e **pannello delle modifiche in sospeso**. Il diff lo produce lo **scrittore vero** (`FileSaverOrchestrator.Righe`, aggiunto al motore) e lo confronta **Myers**, così una modifica esce come una riga tolta e una aggiunta invece che «da qui in giù è tutto diverso». Misura sull'albero: una modifica in ognuno dei **348** file con un campo coordinata scrivibile → **348/348 col diff di una riga tolta e una aggiunta**, 0,8 ms per file; «annulla tutto» in 11 ms e i 754 file tornano **identici**. 🔴 Il «prima» di una modifica è sempre quello **dell'apertura**, non il valore intermedio: annullare deve riportare a un valore che sul disco è esistito. Rimettere a mano il valore di partenza non è una modifica: sparisce dal pannello. Dopo una modifica si rifà la geometria del **solo file toccato** e la mappa riprende quello strato.
- **Slice 7, i vertici**: cambia, aggiungi, togli, e **«incolla da testo»** col convertitore di F1 (archi compresi; un testo con più di un'area si rifiuta dicendo quante sono). Un vertice si scrive **per nome** solo dove il file lo sa riscrivere (i `.tfl`); dove l'elenco è di sole coordinate (`.pol`, `.lairway`) il nome si rifiuta prima di toccare niente. Annullare rimette l'**elenco dell'apertura**, non il gesto di prima. Misura: **154/154** file con elenchi piatti danno il diff di una riga tolta e una aggiunta (0,6 ms per file); l'esempio AIP del committente (arco 17 NM) incollato su `lsas.tfl` fa 1 023 → **89 vertici in 15 ms**. 🔴 Difetto preso **dalla misura, non dai test**: `Punto` ha una conversione **implicita** da `Coordinate`, e nel ternario che sceglieva il tipo il ramo «coordinate» usciva come `Punto` — ogni `.pol` e ogni `.lairway` cadeva con un'eccezione.
- **Slice 7-bis, i tracciati a più tratti** (chiesta dal committente subito dopo la 7): `ElenchiDiVertici` mette d'accordo i **tre** modi in cui il sector scrive «i punti di questa forma» — un elenco di coordinate, un elenco di punti che ammette i nomi, e un elenco di **involucri** che portano il punto più altro (il `PuntoDelTracciato` delle `.sid` con etichetta e «nuovo tratto», i **segmenti** delle zone `.str`, che sono elenchi dentro un elenco). Una zona a più tratti mostra un elenco **per tratto** («Tratto 3») e si modifica un tratto per volta; due tratti toccati sono due modifiche. 🔴 Spostando il punto di una SID l'**etichetta** e il «nuovo tratto» non si perdono, se no il diff toccherebbe righe che nessuno ha chiesto. Albero: **4 468 record** con vertici in **5 892 elenchi** (134 a più tratti, 1 304 tracciati di SID) trovati in 87 ms; «un vertice per file» passa da 154 a **244 file, tutti col diff −1 +1**. 🔴 Di nuovo dalla misura: mancava la guardia sugli **indicizzatori** nel ciclo interno («Parameter count mismatch»).
- **Slice 8, aggiungere e togliere un record** (`IO/RecordNuovo` nel motore): il nuovo **copia il vicino**, così nasce coi campi di struttura giusti e le righe **nella forma dei vicini** (puntata, compatta o decimale); il suo elenco di vertici e' un altro oggetto, se no spostare un punto del nuovo sposterebbe anche il vicino. 🔴 **Togliere un record lascia il commento che ha sopra**: nel sector e' quasi sempre l'intestazione di una sezione — in `itgeo.geo` un taglio da una riga ne faceva sparire tre (lo ha mostrato la misura). L'ultimo record di un file non si toglie. I numeri dei record scorrono: le modifiche in sospeso si rinumerano, quelle del record tolto spariscono; la struttura e' **una voce sola per file** e si annulla per ultima. Misura: **695/695** file con un record aggiunto (422 di una riga sola) e **637/637** con uno tolto danno il diff giusto; «annulla tutto» rimette i 754 file identici.
- **Slice 9, il salvataggio** (`Core/Disco/Salvataggio`): i cinque passi del §2.4 — **confini** e **conflitto d'impronta** su TUTTI i file prima di scriverne uno; **backup** in `%LOCALAPPDATA%\VipiSectorLab\backup\<aaaammgg-hhmmss>\` (una cartella per salvataggio, 30 giorni dal nome, fuori dal clone); **scrittura atomica** coi byte dello scrittore vero (`FileSaverOrchestrator.Byte`/`ScriviAtomico`, aggiunti al motore) e **tre tentativi** se Aurora o un antivirus tengono il file, poi si dice quale; **rilettura** (byte sul disco = byte scritti, impronta nuova, cataloghi e strati rifatti). **Errori nuovi** confrontati per regola e testo della riga (non per numero), contati → conferma, non blocco. **Conflitto** → «Ricarica dal disco», e il diff perso resta leggibile. Albero vero in copia: sposto un fix → **una** riga diversa; un record in ognuno dei 695 file salvato → backup rimesso = **754/754** identici. 🔴 La misura ha preso un difetto della **slice 8**: nei `.mva`/`.lartcc`/`.hartcc` un record è un blocco chiuso dalla riga vuota, e la copia accostata al vicino riletta ne era un pezzo (63 file su 695): ora prende la riga vuota dei vicini; nei `.vrt`/aerovie (separa la chiave, 13 file) il salvataggio lo dice e chiede conferma. 🔴 La copia di prova col solo nome del file perdeva `/ENRMVA/` (lettore sbagliato): ora porta il percorso intero.
- **Slice 10, il pannello dei problemi**: il validatore dell'albero in una linguetta accanto a Sfoglia e nella barra, **fuori dal circuito** (1,5 s all'apertura e dopo ogni salvataggio, vale l'ultimo giro); percorsi del validatore (`Include\IT\…` relativi a SectorFiles) tradotti in quelli della sessione in un posto solo; clic → il record della riga (`RecordDellaRiga`: righe di dati, non commenti) nell'ispettore e sulla mappa con la riga segnata, o le righe del disco se la riga non è di un record; filtro; «le modifiche in sospeso introducono 1 errore» dopo ogni gesto (27 ms sul circuito nel caso peggiore, il resto fuori). Albero vero: 483 (130/353), 483/483 file ritrovati, 379 record agganciati tutti giusti.
- **Prove a mano del committente** (fork `SkyMistery/it-aurora-sector-test` clonato in `D:\Programmazione\IVAO_Test\it-aurora-sector-test`, eseguibile in `SectorLab-prova\` con `PROVE.md`): ✅ **Aurora carica il sector salvato dal Lab** (fix, vertici, incolla AIP). Corretti: 🔴 i **`T;dummy;` minuscoli** (100 righe in 4 file) letti come vertici → 53 linee a N0 E0 sulla mappa, ora separatori come in Aurora (tre lettori del motore; il lettore del sito lo faceva già); la **mappa segue la scelta** (accende lo strato del record, reinquadra al secondo clic, segue il record spostato); il **registro** `%LOCALAPPDATA%\VipiSectorLab\log\lab-aaaammgg.txt` (14 giorni: gesti, esiti, errori con stack, anche del framework e non gestiti) col tasto «Registro». Seconda tornata: ✅ **tutte e dieci le prove**; l'«incolla da testo» ora disegna gli archi fitti come quelli del file (stima dagli archi esistenti, campo «un punto ogni N°»: `LIRN_TWR` 4,3° invece di 1°) e conserva la chiusura della forma. Test Lab 262, motore 427.
- ▶ **Prossima: slice 11, la consegna** (workflow `sectorlab-v*`, release, avviso di versione, scheda vIPI). Poi 12 chiusura.
- 🔴 **Consegna decisa col committente**: release **GitHub pubblica** (tag `sectorlab-v*`, costruita dalla CI su runner Windows) con il link in una scheda `/services` visibile a **Editor e Admin** (carta §2.5). Il repo è pubblico e lo zip sarà scaricabile da chiunque: detto e accettato («va bene così per il tipo di progetto che è»); vIPI privata scartata per il costo della CI (~14 000 minuti/mese contro 2 000-3 000 inclusi) e per il submodule di Ivao.It.
- 🟡 Aperto: un rosso intermittente **visto una volta**, `Vipi.Ui.Tests.PaginaAuditUnGiroAllaVoltaTests.Due_cambi_di_periodo_ravvicinati_non_sovrappongono_i_caricamenti` (net10, `WaitForElement` a 3 s), verde al rilancio — §A116.
- 🟡 **§A117, in PR a parte (non in `main`)**: la vista **`v_share_atc_sessions`** per l'IVAO Division Hub, migrazione **solo MySQL** `VistaCondivisaSessioniAtc` (carta `docs/feature/2026-09-23-vista-condivisa-sessioni-atc.md`). È un **contratto**: cambia solo aggiungendo colonne. La copia del database ora porta le viste `v_share_` (senza, si sarebbe rifiutata). ▶ Prima del pacchetto: l'utente della vIPI su Plesk deve poter **creare viste** (da verificare, o la migrazione ferma l'avvio); il `GRANT SELECT` all'utente dell'hub lo fa a mano chi amministra il server.
- ⚠️ In produzione nulla di nuovo dopo **1.42.1**.

Prima: 22 settembre 2026 (sera) — ✅ **Aurora Sector Lab F2 FATTA** (§A115 chiusa, carta `docs/feature/2026-09-22-f2-motore-del-sector.md` ✅). **Slice 9** (`0240aa11`): la concordanza del motore con `AuroraSectorfileParser` (`tools/Vipi.SectorfileProva/Concordanza.cs`, collegato anche in `Vipi.Infrastructure.Tests`) — sull'albero intero punti 4 048, SID 1 515, STAR 863 concordi, 0 discordi; il motore sbagliava il **TACAN di Grosseto** (`itvor.vor:81`, frequenza vuota: corretto, non è un errore del sector); vIPI non legge la coppia decimale di `MIL.fix:235` `TAC-06R` (**non corretto**, l'import non cambia in F2: resta aperto in §A115). **Slice 10**: chiusura, e il **messaggio per gli AOD** con tutti gli errori di F1 e F2 nel **§10 della carta**, pronto da incollare. Test: Sectorfile 419, Infrastructure 1 596. Nessun codice del sito toccato, niente pacchetto. ▶ Prossimo del Lab: **la carta di F3** (l'app). ✅ Anche il secondo chief (LIBB-CH) è comparso in Diagnostica (confermato dal committente): §A114 chiusa del tutto.

Prima: 22 settembre 2026 (pomeriggio) — 🟡 **Aurora Sector Lab F2 a due slice dalla fine** (§A115, carta `docs/feature/2026-09-22-f2-motore-del-sector.md`): slice 0-8 fatte, ultimo `3479a52a`, CI verde, working tree pulito. Dopo la 5: **6** lettori nuovi `.hold`/`.vrt`/aree P/R/D; **7** i tag `//@` di `.sid`/`.str` (`IO/Metadati.cs`, lettura e scrittura, aggancio col nome); **8** il **validatore** (`Validazione/`, regole di un file e dell'albero coi cinque `.isc`): **131 errori e 352 avvisi** sul master del sector, tabelle per gli AOD nella carta (slice 5, 6, 8). ▶ **Si riparte da «Per la ripresa: slice 9 e 10» in fondo alla carta**: la 9 è la concordanza SID/STAR e punti col lettore di vIPI (`AuroraSectorfileParser`), la 10 la chiusura. Nessun codice del sito toccato, niente pacchetto; in produzione nulla di nuovo dopo 1.42.1. ▶ Ancora aperto: il secondo chief (LIBB-CH) comparso in Diagnostica?

Prima: 22 settembre 2026 (mattina) — 🟡 **Aurora Sector Lab F2 in corso** (§A115, carta `docs/feature/2026-09-22-f2-motore-del-sector.md`): slice 0-5 fatte, ultimo `57cea85c`, CI verde. Il motore `src/Vipi.Sectorfile` legge e riscrive l'albero intero del sector (681/681, una modifica per record = una riga, un campo); righe opache 7 579 → 7, che sono **errori veri del sector da dire agli AOD** (elenco in §A115). Nessun codice del sito toccato, niente pacchetto. ▶ Prossima: slice 6 (`.vrt`, `.hold`, aree P/R/D). In produzione nulla di nuovo dopo 1.42.1.

Prima: 22 settembre 2026 (notte) — ✅ **1.42.1 È ONLINE** (§A114: timbro, `Schema 0` e il primo chief in Diagnostica confermati dal committente; da fuori 8/8): PATCH, **nessuna migrazione**, **4 file in radice** (`Vipi.Application` + `Vipi.Host`, dll e pdb), timbro `1.42.1 · a66f25e`, zip `f76baed2…82059`, foglio `deploy/atc-ivao/LEGGIMI-PACCHETTO-1.42.1.md`. I **chief d'ACC** (`LIxx-CH`) entrano nel roster degli staffisti: erano Redattori ma invisibili in Diagnostica e nel selettore dei permessi, perché il roster accettava solo i codici `IT-`. `…-CHA1` resta senza permessi da staff: si promuove a mano (decisione del committente). In mezzo, già online: 1.40.0 (net10), 1.41.0/1.41.1 (documenti collegati), 1.42.0 (Sector Lab F1) — dettaglio in `docs/lavori-aperti.md` §A107–§A113.

Prima: 21 settembre 2026 (notte, ultima) — ✅ **1.39.1 È ONLINE** (§A105): il committente conferma timbro 1.39.1, `Schema 0` e sommari funzionanti. Da fuori `pacchetto-verifica.js` verde e `gruppi-toc-verifica.js` verde sulla vIPI pubblica di LIBB (un clic su «Settori di aerovia» chiude tutto il gruppo). Porta §A103: nel sommario un gruppo intestato (documento di una pagina unita, blocco della vIPI ACC) si chiude e si apre tutto intero. In `main` non resta codice fuori pacchetto. 🔎 Resta aperto da 1.39.0: SCCAM/FIC «No sector» nella vIPI pubblica di LIBB (vedi sotto). ▶ Ripubblicare vIPI ACC e scali. 🔴 **net10 = 1.40.0, DA SOLA** (scelta del committente: un guasto, una causa).

Prima: 21 settembre 2026 (notte, fine) — ✅ **1.39.0 È ONLINE** (§A102): il committente conferma timbro, `Schema 0`, colori MIL verdi / FSS petrolio, STAR nascoste. Da fuori `pacchetto-verifica.js` 8/8 e `mappa-foglio-verifica.js` 8/8 sulla vIPI pubblica di LIBB (mappe intere anche arrivando da un'altra pagina). 🔎 **Alla ripresa**: la vIPI pubblica di LIBB dice «No SCCAM/FIC sector in this ACC» — nell'editor di produzione guardare se i settori MIL/FSS di LIBB ci sono (se sì, ripubblicare; se no, anagrafica settori). ▶ Resta da ripubblicare vIPI ACC e scali. In `main` non resta codice fuori pacchetto. 🔴 **net10 = 1.40.0.**

Prima: 21 settembre 2026 (notte, tardi) — 📦 **1.39.0 PRONTO DA CARICARE** (§A101): MINOR con **UNA migrazione di soli dati** (`StarNascosteDiDefault`: le STAR degli aeroporti nascoste; → **copia del database prima**, `MySqlMigrations.dll` DENTRO), **12 file** (Application, Infrastructure, MySqlMigrations, Host, `vipi-aor.js` con `.br`/`.gz` + `endpoints.json`), timbro `1.39.0 · 1b25c4f`, zip `ba3ad79a…028af`. Porta §A98 (MIL/FSS fuori dalle configurazioni, MIL verdi, FSS petrolio), §A99 (mappe a pezzi arrivando da un'altra pagina: `leaflet.css` nel `<head>`), §A100 (STAR nascoste). Provato sul pacchetto: 10/10, mappe 8/8 su vIPI ed editor, STAR 4/4, famiglie AoR pulite, migrazione nel log. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, riga della migrazione nel log; poi **ripubblicare le vIPI ACC e gli scali**. 🔴 **net10 = 1.40.0.**

Prima: 21 settembre 2026 (notte) — ✅ **1.38.0 È ONLINE** (§A97): il committente conferma timbro, `Schema 0`, SCCAM/FIC e STAR richiamate nei trasferimenti; da fuori `pacchetto-verifica.js` 8/8. ▶ **Il committente ha «una cosa da correggere»: la dice nella chat nuova — chiederla per prima.** 🔎 Visto da fuori: nella vIPI pubblica di LIBB le sezioni nuove sono **accodate in fondo** ai blocchi e il VFR del gruppo APP è ancora quello vecchio — è la rete a view-time su una release non ripubblicata; col login verificare l'ordine nell'editor e le righe dei passi d'avvio nel log di produzione (dettagli §A97). In `main` non resta codice fuori pacchetto. 🔴 **net10 = 1.39.0.**

Prima: 21 settembre 2026 (sera, fine) — 📦 **1.38.0 PRONTO DA CARICARE** (§A96): MINOR, **nessuna migrazione EF** ma i passi d'avvio scrivono nei documenti (→ **copia del database prima**), **15 file** (Application, Infrastructure, Hosting, Ui + `en/`, Host per il timbro, `vipi-editor.js` con `.br`/`.gz` + `endpoints.json`), timbro `1.38.0 · d267c1d`, zip `00d3d964…979a`. Porta §A88–§A95. Provato sul pacchetto: 10/10, `Schema 0`, passi d'avvio nel log, SCCAM/FIC, gruppo APP nuovo, otto chip. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, poi **ripubblicare le 4 vIPI ACC** e scrivere IFR/Tecnica operativa dei gruppi APP; STAR fra i punti da guardare in produzione. 🔴 **net10 = 1.39.0.**

Prima: 21 settembre 2026 (sera, dopo) — altri quattro lavori in `main`, CI verde, sempre **fuori pacchetto**: **§A92** sezioni MIL/FSS rinominate **SCCAM** e **FIC**; **§A93** SID/STAR nei trasferimenti **col nome di oggi** dell'archivio (come tabelle e citazioni: «BANA9A» → «BANAV 9A», rinominata → segue da sola); **§A94** il **gruppo APP della vIPI ACC come l'APP non remotizzato** («Gestione del traffico» con IFR/VFR a blocchi, «Tecnica operativa»; il VFR esistente spostato e travasato all'avvio); **§A95** **`[[AREA …]]`** nel «Cita», le aree regolamentate col nome di oggi. ▶ Pacchetto MINOR quando chiesto (net10 → 1.39.0 se è 1.38.0); dopo il carico **ripubblicare le 4 vIPI ACC**. Dettaglio in `docs/lavori-aperti.md` §A88–§A95.

Prima: 21 settembre 2026 (sera) — ✅ **1.37.0 È ONLINE** (timbro e `Schema 0` confermati dal committente). 🟡 **In `main`, NON in pacchetto, nessuna migrazione: §A88–§A91**, CI verde. **§A88** la Diagnostica lenta (i tempi mandati: controlli 708 ms): `BuildClaims` rianalizzava i poligoni a ogni coppia settori/quota, ora memoria dei volumi per riga — analisi 491 → 184 ms; il doppio giro del prerender resta per scelta del committente. **§A89** «Cita» scrive dove sta il cursore anche se il campo ha perso il fuoco. **§A90** vIPI ACC: settori **MIL** e **FSS** fuori dall'AoR in cima, ciascuno nella sua sezione con la sua mappa (MIL prima delle aree regolamentate, FSS dopo); le vIPI esistenti le ricevono all'avvio nel posto del catalogo. **§A91** trasferimenti: **SID/STAR fra i punti**, frase «autorizzato via *procedura*» e luogo di default **al confine dell'AoR**, suggerite nell'editor dagli scali della sezione. ▶ **Pacchetto quando il committente lo chiede**: è un **MINOR** (se 1.38.0, net10 → 1.39.0). ▶ Dopo quel carico: **ripubblicare le quattro vIPI ACC** perché le sezioni MIL/FSS compaiano in pubblico; le STAR fra i punti si provano in produzione (in locale non ci sono). ▶ Aperti ereditati: resa a mano delle due frasi ferme, ripubblicare per la sezione STAR, il prossimo scarico di `diagnostica/` per §A83/§A84.

Prima: 21 settembre 2026 (pomeriggio) — 📦 **1.37.0 PRONTO DA CARICARE** (§A87): MINOR, **nessuna migrazione**, **14 file** (Application, Ui + `en/` resources, Host per il timbro, `vipi-theme.css` e `vipi-aor3d.css` con `.br`/`.gz` + `endpoints.json`), timbro `1.37.0 · 2ad1790`, zip `fa63471c…0d6a`. Porta **§A86**, cinque segnalazioni dal campo: linguette 2D/3D della AoR che nascevano nude (regole spostate dal foglio pigro al tema, presidio nuovo), **Diagnostica a fisarmonica**, `Re-import from IVAO` che importa anche **SID+STAR** (e «+ STAR» sugli arrivi), **`[[POS LIRR_NE]]`** accanto a `[[ATC …]]`, e la Diagnostica che **dice quanto ci mette** pezzo per pezzo (la lentezza in produzione non si misura da qui). Provato sul pacchetto: 10/10, `Schema 0`, fisarmonica 7/7, linguette vestite anche bloccando il foglio pigro, «admin 0 ms». ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, e **farsi mandare i quattro tempi** della Diagnostica. ▶ N+1 degli orfani in `sector-structure` noto e non toccato. 🔴 **net10 = 1.38.0.**

Prima: 21 settembre 2026 — ✅ **1.36.0 È ONLINE** (§A85). Il committente conferma **timbro corretto** e riga **`Schema` = `0`** (la migrazione è entrata, `MySqlMigrations.dll` è arrivato). Da fuori subito dopo: `pacchetto-verifica.js` **8/8** con la Ricerca, console pulita. 🔴 **La prova vera non è una schermata: è il PROSSIMO scarico di `diagnostica/`, e si legge per ASSENZA** — in `errori-richieste.txt` la catena **`DocReviewBar` non deve più comparire** (contando **per era**, regola 5); in `avvisi-log.txt` **una riga sola** «*N segmenti hanno smesso di partire dopo 3 tentativi*» e mai ripetuta; nei `log-*.txt` le righe «*caratteri spesi per N segmenti tornati rotti*» devono **azzerarsi** (erano ~70-130 al giorno). ⚠️ Serve **una giornata** di traffico: un'era con quattro richieste non assolve nessuno (regola 6). ▶ Resta lavoro di **DATI**: la resa a mano delle due frasi ferme (`«37th WING …»` seme `FrasiAreeIvao`, `«Se presente LIBN_G_APP …»` pannello traduzioni) e **ripubblicare i documenti** per la sezione STAR di 1.35.0. In `main` non resta codice fuori pacchetto. 🔴 **net10 = 1.37.0.**

Prima: 21 settembre 2026 (notte) — 📦 **1.36.0 PRONTO DA CARICARE** (§A85): **MINOR** con **UNA migrazione** `FrenoTraduzioneQuarantena` (tabella `TranslationQuarantines`, nuova), **12 file tutti in radice**, timbro `1.36.0 · 0493ef5`, zip `d9cc681c…9475`. Porta due correzioni uscite dalle **diagnostiche del 21 settembre**: **§A83** — `DocReviewBar` prende la **terza porta** (era la catena più nominata di `errori-richieste.txt`, **40 voci**, l'**unico 500** del periodo il 19-set alle 19:48 a un utente vero, e l'**unica** voce dell'era 1.35.0; aveva già scope proprio e sentinella, le mancava l'**attesa**; tolta dal debito di `TerzaPortaTests`, 21 → 20) — e **§A84**, il **freno della traduzione** (due frasi bruciavano **170 506 caratteri in cinque giorni**, ~1 M al mese, in **410 spedizioni tornate rotte tutte e 410**: dalla terza rottura il segmento **smette di partire**). 🔴 **Prima del carico: copia di sicurezza del database**, ed è la **seconda consegna con migrazione di fila**; ⚠️ senza `MySqlMigrations.dll` qui non si rompe niente — il freno semplicemente **non frena**, e lo dice solo la bolletta. Provato sul pacchetto (publish win-x64, :5199, copia del `vipi.db`): **10/10** con la Ricerca, migrazione applicata davvero nel log, **`Schema 0`**, editor con la barra che si apre. ✅ Niente `wwwroot` e quindi **niente `endpoints.json`**; fuori anche `en/Vipi.Ui.resources.dll` (nessuna frase cambiata). ▶ Dopo il carico: timbro, Ricerca, `Schema 0`. ▶ Resta lavoro di **DATI**: la resa a mano delle due frasi ferme, e ripubblicare i documenti per la sezione **STAR**. ✅ Dal registro, la buona notizia: **1.34.3 ha chiuso con ZERO errori su 5990 richieste**. 🔴 **net10 slitta a 1.37.0.**

Prima: 21 settembre 2026 — ✅ **1.35.0 È ONLINE** (§A82). Il committente conferma: copia di sicurezza del database **scaricata prima** del carico, timbro `1.35.0` in barra, riga **`Schema` = `0`** (la migrazione è entrata, `MySqlMigrations.dll` è arrivato). Da fuori subito dopo: `pacchetto-verifica.js` **8/8** con la Ricerca, console pulita, e `vipi-theme.css` servito con lo **sha256 identico** a quello spedito (`b64a6ede…b193`). ▶ **Resta, ed è lavoro di DATI non di codice: ripubblicare i documenti** perché la sezione «STAR» compaia nelle copie pubbliche (nella bozza c'è già); le STAR importate aspettano il **ciclo d'entrata** (LIBD `2610`). ▶ Col login, quando capita: tabella STAR, tasto «Cita» a sei chip e campo di prosa che si allunga, dopo Ctrl+F5. ▶ Aperti ereditati: resa a mano di **LIBN_APP**, e `ConnectionError` di EF su MariaDB da tenere d'occhio. 🔴 **net10 = 1.36.0**.

Prima: 21 settembre 2026 — 📦 **1.35.0 PRONTO DA CARICARE** (§A82): MINOR, **UNA migrazione** `ProcedureKindEStarNellaStessaTabella` (`AirportSids` → `AirportProcedures` + colonna `Kind`), **17 file** con `Vipi.Infrastructure.MySqlMigrations.dll`, timbro `1.35.0 · b38359b`, zip `eb4cad97…4381`. Porta **§A80** (le **STAR** dal sectorfile: parser dei `.str`, import, editor, sezione «STAR» nel documento, `[[STAR …]]`; e i **riferimenti ai dati** `[[FREQ]]` `[[ATC]]` `[[RWY]]` `[[FIX]]` con un selettore solo a **sei** chip e il tasto «Cita»), **§A81** (le nove correzioni della review, due sul percorso Postgres) e **§A82** (il campo di prosa che torna ad adattarsi al testo: lo scavalcava `.app-block-edit .app-ta{flex:1}`, perché in un flex a COLONNA `flex:1` vale `flex:1 1 0%` sull'ALTEZZA). 🔴 **Prima del carico: copia di sicurezza del database** — è la prima consegna con migrazione dal 1.27.0. Provato sul pacchetto (publish win-x64, :5199, copia pre-migrazione): 10/10 con la Ricerca, la migrazione applicata davvero nel log, STAR + «Cita» a sei chip + il campo che cresce sul CSS minificato. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, e in un editor dopo Ctrl+F5 la tabella STAR e il tasto «Cita». ▶ Restano: la sezione «STAR» nei documenti **pubblicati** solo dalla prossima release, le STAR in attesa del **ciclo d'entrata** (LIBD `2610`), la resa a mano di LIBN_APP, e `ConnectionError` di EF su MariaDB da tenere d'occhio. 🔴 **net10 = 1.36.0**.

Prima: 18 settembre 2026 (sera) — 📦 **1.34.0 PRONTO DA CARICARE** (§A74): MINOR, **nessuna migrazione**, **16 file** (Application, Infrastructure, Ui + `en/` resources, Host per il timbro, `vipi-editor.js` e `vipi-theme.css` con `.br`/`.gz` + `endpoints.json`), timbro `1.34.0 · 9d3530e`, zip `ceca7692…801a`. Porta **§A73, le SID citate nel testo**: riferimento `[[SID ICAO NOME]]` in prosa e celle, nome che segue la SID quando si aggiorna (completo, «BANAV 9A»), tasto «SID» nell'editor, avviso delle citate da ricontrollare, conversione di quelle scritte a mano, riferimenti e link agli allegati protetti dalla traduzione, voce nella Guida; più la **callout con titolo** che non entrava nell'editor. Rivisto da due revisori indipendenti (un rilievo alto: DbContext condiviso dall'editor, corretto). Provato sul pacchetto su copia prod in MariaDB: 10/10 con la Ricerca, conversione su LIBV e tasto SID su LIBD verdi. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, tasto «SID» in un editor dopo Ctrl+F5. ▶ Agli AOD: la vSOP MIL di **LIBV cita revisioni vecchie** (`ROBOT6A/B`, `DOGUS5A/B`). ▶ RFO: il 401 era il file `Rfo` **mai messo** in `segreti/` (non il riavvio); messo il 18-set, manca il curl 404 di conferma. 🔴 net10 = **1.35.0**.

Prima: 18 settembre 2026 — ✅ **1.33.0 È ONLINE** (§A72 + §A69): ponte **RFO Gate Manager** `GET`/`PUT /api/rfo/events/{eventId}/state` per l'evento LIRN del 19 settembre (carta `docs/feature/2026-09-18-ponte-rfo-gate-manager.md`), migrazione additiva `PonteRfo`, timbro `1.33.0 · 7d25267`, zip `0651b7ca…`. Da fuori: sito 8/8, endpoint vivo, Cloudflare lascia passare. 🔴 **Chiave ancora rifiutata (401)**: il file in `segreti/` si legge SOLO all'avvio → il committente riavvia, poi curl + PUT paralleli su `prova-ponte-rfo` (mai sull'evento vero) e, col login, timbro e `Schema 0`. Poi si torna a §A70. net10 = **1.34.0**.

Prima: 17 settembre 2026 (notte) — ✅ **1.32.0 È ONLINE** (§A68: timbro e `Schema 0` confermati, colonna «Mai usare» presente in vIPI e vSOP). 🟡 **§A69 in main, NON in pacchetto**: le caselle DEP/ARR diventano chip `sh-chip` come le colonne accanto, **rosse** quando accese (`sh-chip.no`); solo `Vipi.Ui` + `vipi-theme.css`, nessuna migrazione. Test bUnit provati per mutazione, verifica dal vivo in scuro/chiaro e sull'editor militare, contrasto 5,56:1. ▶ **Va col prossimo pacchetto** (decisione del committente): niente PATCH a parte. ▶ **Si riparte da §A70 di `docs/lavori-aperti.md`** (ripubblicare LIBA_APP e gli APP agganciati pubblicati sotto 1.31.0; aperti ereditati da §A64). 🔴 net10 = **1.33.0**.

Prima: 17 settembre 2026 (sera, fine) — ✅ 1.31.2 online; 📦 **1.32.0 PRONTO DA CARICARE** (§A68): MINOR, **una migrazione additiva** `PisteMaiUsarePerVerso` (due colonne booleane in `AirportRunways`), 17 file con `Vipi.Infrastructure.MySqlMigrations.dll`, timbro `1.32.0 · c1eddc9`, zip `e94fcd9d…c4de`. Nella tabella Piste degli editor aeroporto e militare ogni soglia si marca **«mai in partenza»** e/o **«mai in arrivo»**: quando nessuna regola pista vale il ripiego sul vento non la sceglie in quel verso; le regole restano valide (l'editor avvisa); invisibile al lettore; il **vAWOS lo legge dal pubblicato** come regole e LVP. Guida aggiornata (AoR spazi aerei, piste). Provato su copia fresca della produzione: migrazione all'avvio, 10/10, `Schema 0`. ▶ **Prima del carico: copia del database.** ▶ Dopo: timbro, Ricerca, `Schema 0`, colonna Mai usare dopo Ctrl+F5. Vista rapida ed elenco aeroporti ora decidono anche loro sul pubblicato (`IPisteDalPubblicato`, porta unica con il vAWOS). 🔴 net10 = **1.33.0**.

Prima: 17 settembre 2026 (sera, tardi) — ✅ **1.31.2 È ONLINE** (§A67: timbro e `Schema 0` confermati, stile dei campi confermato dal committente). In main non resta codice fuori pacchetto: PATCH, 8 file, timbro `1.31.2 · 2c64512`, zip `8d2aafbc…b370`, nessuna migrazione. Sette campi dei 5 editor vestiti dal browser (note e classe della tabella Spazi aerei, note Aree di lavoro/BOAT, filtro aree regolamentate, campi `.se-row`, tendine radioassistenze/alternati, freccia delle tendine in `.struct .res-table`) portati allo stile di casa, trovati con `campi-verifica.js` (566 campi, scuro e chiaro → 0). Provato sul pacchetto 10/10. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, tabella Spazi aerei in modifica dopo Ctrl+F5. 🔴 net10 = **1.32.0**.

Prima: 17 settembre 2026 (sera) — ✅ 1.31.0 online; 📦 **1.31.1 PRONTO DA CARICARE** (§A66): PATCH, 20 file, timbro `1.31.1 · ab20c46`, zip `ef65da85…c2cd`, nessuna migrazione. Nella tabella **Spazi aerei (AIP)** sotto l'AoR un quadratino per riga accende e spegne il singolo volume sulla mappa 2D e 3D; hover riga ↔ poligono con nome e quote. Provato sul pacchetto: 10/10, sonda `spazi-aerei-verifica.js` verde, anche in pubblico dopo ripubblicazione. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, prova su LIBP_APP dopo Ctrl+F5; ripubblicare gli APP agganciati pubblicati sotto 1.31.0 (LIBA_APP). 🔴 net10 = **1.32.0**.

Prima: 17 settembre 2026 (pomeriggio, tardi) — 📦 **1.31.0 PRONTO DA CARICARE** (§A65): MINOR, 13 file, timbro `1.31.0 · 68265f9`, zip `d9de8fc5…b54a`, nessuna migrazione. Porta la tabella **Spazi aerei (AIP)** sotto la mappa AoR di vIPI APP e ACC per i settori agganciati al KMZ: nome, base, tetto, classe e note, con classe e note scrivibili da chi aggiorna il documento. Provato sul pacchetto su copia prod in MariaDB: 10/10, tabella in editor, bozza e pubblico. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, tabella su LIBP_APP dopo Ctrl+F5; **ripubblicare LIBA_APP**. 🔴 net10 = **1.32.0**.

Prima: 17 settembre 2026 (pomeriggio) — ✅ **1.30.4 È ONLINE** (§A63: timbro e `Schema 0` confermati, da fuori verde, `vipi-editor.js?v=54914c5f` servito; ▶ **DA FARE in §A64** di `docs/lavori-aperti.md`: riprova ILS dell'admin, due righe di traduzione a giro al prossimo scarico, poi i lavori aperti): PATCH, 10 file, timbro `1.30.4 · a10d350`, zip `0d5f3637…0404`, nessuna migrazione. ✅ 1.30.3 online, registro del giorno funzionante. Dentro: **§A61** radioassistenze, «ILS» scelto dal suggerimento faceva morire la pagina (due `change`, due scritture insieme sul DbContext): scritture in fila + JS che mangia il doppione; **§A62** il giro traduzioni faceva 8 passate invece di 2 (il legame della configurazione raddoppiava `Targets`/`Order`), segmenti rotti ripagati ×4. Provato sul pacchetto; la sonda `ils-verifica.js` riproduce lo stack di produzione sul codice di 1.30.3 e passa su 1.30.4. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, riprova ILS dell'admin dopo Ctrl+F5, due righe di traduzione a giro in `log-*.txt`. 🔴 net10 = **1.31.0**.

Prima: 17 settembre 2026 (mezzogiorno) — 📦 **1.30.3 PRONTO DA CARICARE** (§A60): PATCH, 2 file (`Vipi.Host.dll`+`.pdb`), timbro `1.30.3 · b5be0ff`, zip `de4a8db6…bcee`, nessuna migrazione. Porta §A59, il registro del giorno in `diagnostica/` (`richieste-*.tsv`, `log-*.txt`, sette giorni). Provato sul pacchetto su copia prod in MariaDB: 10/10, file nati giusti. ▶ Dopo il carico: timbro, Ricerca, `Schema 0`, i due file del giorno via FTP. 🔴 net10 = **1.31.0**.

Prima: 17 settembre 2026 (tarda mattina) — 🟡 **§A59 in `main`, NON in pacchetto, nessuna migrazione**: il **registro del giorno** in `diagnostica/` — `richieste-AAAA-MM-GG.tsv` (una riga per richiesta: rotta dall'endpoint, esito, ms, pid, versione, autenticato; niente query, ping, statici) e `log-AAAA-MM-GG.txt` (righe `Vipi.*` da Information), sette giorni, tetto 5 MB per file; si leggono con `python tools/registro-del-giorno.py`. Carta `docs/feature/2026-09-17-registro-del-giorno.md`. Provato dal vivo. 🔴 `/vsop/live/atc` da 50 s NON è lentezza: è lo stream SSE. ▶ Pacchetto quando chiesto: `Vipi.Host` dll+pdb.

Prima: 17 settembre 2026 — ✅ **1.30.2 È ONLINE** (§A57): `Schema 0` confermato, da fuori verde con la Ricerca, `vipi-theme.css` con sha256 identico. Timbro `1.30.2 · 76aceb3`, 8 file, zip `bca3cdcf…a9a3`, PATCH senza migrazioni. Dentro: **§A54** Diagnostica a zoom 175% non taglia più la scheda della copia del database; **§A55** il guasto del primo `avvisi-log.txt` (16-set 20:44Z, `DbUpdateConcurrencyException` nel ponte delle forme) erano **due processi Passenger** con la stessa scadenza d'import — ora il ponte rilegge dall'archivio e riprova, chi salva non cade; **§A56** la chiave Azure nuova risponde solo sull'endpoint della risorsa `ivao-it-translator` (col percorso `/translator/text/v3.0`), che il motore non sapeva usare. ▶ **Da confermare**: `Translation:Azure:BaseUrl` nel file dei segreti; al prossimo scarico niente `HTTP 401` in `avvisi-log.txt` e niente «Pezzi di forma disallineati» → poi **S11 fase B**. ℹ️ Il ping ogni minuto su `/vsop/health/ready` è il NOSTRO keep-alive (Worker `atc-archiver`): resta. ▶ Aperti: segmenti che Azure rende rotti e si ripagano («IAFs ILS14», «37th WING A/A TRAINING AREA»); ripieghi shape dell'import che falliscono a `LogDebug`; punto LOVV con secondi = 60 nel sectorfile (loro); feedback a due canali. In main non resta codice fuori pacchetto. 🔴 net10 = **1.31.0**.

Prima: 16 settembre 2026 (notte, fine) — ✅ **1.30.1 È ONLINE** (§A53): timbro confermato, da fuori verde con la Ricerca. In main non resta codice fuori pacchetto. ▶ Prossimo scarico di `diagnostica/`: guardare `avvisi-log.txt`. ▶ S11 fase B dopo giorni di Diagnostica senza «Pezzi di forma disallineati». 🔴 net10 = **1.31.0**.
...
Prima: 16 settembre 2026 (notte, dopo il pacchetto) — 📦 **1.30.0 PRONTO DA CARICARE** (§A51): timbro `1.30.0 · d083a15`, zip `713324d5…17f5`, **15 file**, MINOR con **una migrazione distruttiva** (via `Airports.IsMilitaryOnly`) + S11 fase A. Provato sul pacchetto su una copia fresca della produzione in MariaDB: migrazione applicata, 280 settori allineati, pacchetto 10/10, `Schema 0`. ▶ PRIMA del carico: copia del DB scaricata e verificata INTERA. ▶ Dopo: timbro, `Schema 0`, nessun «Pezzi di forma disallineati» (anche dopo qualche giorno). 🔴 net10 = **1.31.0**.

Prima: 16 settembre 2026 (notte, tardi) — 🟡 **§A50: S11 approvata, fase A in `main`** (NON in pacchetto, nessuna migrazione): il ponte `PonteDelleForme` copia colonne → `SectorShapeParts` a ogni salvataggio e a ogni avvio, rilievo di Diagnostica sui disallineati, precedenza dell'archivio rovesciata. A schermo niente cambia. Provata sulla copia di produzione su MariaDB (280 settori, poi zero). ▶ Online dopo il carico: Diagnostica senza «Pezzi di forma disallineati», poi fase B (letture). Il pacchetto porta anche §A49 (migrazione distruttiva).

Prima: 16 settembre 2026 (notte, dopo il clear) — 🟡 **§A49 in `main`, NON in pacchetto, con una migrazione DISTRUTTIVA** (`SpecchioSoloMilitareInPensione`: via `Airports.IsMilitaryOnly`, misurato innocuo sulla copia di produzione; copia fresca del DB prima del carico). **S11 della shape riletta**: il piano di §4-bis era corto (16 letture, 5 scritture di quote, precedenza dell'archivio da rovesciare) — ▶ il committente decide se farla o parcheggiarla. ▶ Keep-alive via cron: i ping corti sono già stati misurati inutili (4-set); da provare semmai una connessione lunga. 🔴 net10 = **1.30.0**.

Prima: 16 settembre 2026 (notte, fine giornata) — ✅ **1.29.0 È ONLINE** (§A48): timbro `1.29.0 · 49a2dd5` confermato, copia del database di produzione scaricata, INTERA e ripristinata in locale. 🔓 **La finestra cieca è CHIUSA** (conferma del committente): cancellati `MigrazioniDellaFinestraCiecaTests` e la deroga `RevisionateAMano`, aggiornati commenti, spec e runbook (§A42 punto 2). Una migrazione distruttiva ora si può spedire, con gli occhi aperti e una copia fresca del database prima. ▶ Col login resta `Schema: 0` e la ★ blu su un APP. ▶ **Da fare (§A42)**: S11 della shape (rileggere §4-bis prima), feedback a due canali; possibile pulizia: togliere `Airports.IsMilitaryOnly`. 🔴 net10 = **1.30.0**. In main non resta codice fuori pacchetto (solo la rimozione del test e commenti).

Prima: 16 settembre 2026 (notte, tardi) — ✅ **1.29.0 È ONLINE** (`docs/lavori-aperti.md` §A48): timbro `1.29.0 · 49a2dd5` confermato in barra; la **copia del database scaricata dalla produzione** (55 MB) è INTERA (62 tabelle, 79.053 righe) e **si ripristina** in un database vuoto con le stesse righe e 54/54 immagini integre; da fuori pacchetto verde e CSS con sha256 identico. ▶ Col login resta `Schema: 0` e la ★ blu su un APP. In main non resta niente fuori pacchetto. 🔴 net10 = **1.30.0**. ▶ **A42 da fare**: S11 shape, presidio della finestra (da confermare), feedback a due canali.

Prima: 16 settembre 2026 (notte, dopo la revisione) — 📦 **1.29.0 PRONTO DA CARICARE** (`docs/lavori-aperti.md` §A48): timbro **`1.29.0 · 49a2dd5`**, zip `f3d84204…ba38`, **15 file**, MINOR su 1.28.0, **nessuna migrazione**. Dentro §A45 (★ blu), §A46 (intro MIL chiusa), §A47 (copia del database da Diagnostica, rivista da un revisore indipendente: tre difetti sul ripristino corretti, e uno **stack overflow latente** in `ScopeProprioCheAspetta.InFilaAsync<T>` che avrebbe spento il sito all'apertura della Diagnostica). 🔴 `Vipi.Host.dll` si spedisce per il **timbro**. Provato sul pacchetto su MariaDB: pacchetto 10/10, copia 11/11, file scaricato INTERO. ▶ Dopo il carico, col login: timbro, `Schema: 0`, Diagnostica che resta su, download + `tools/Vipi.DbBackup verifica`. 🔴 net10 = **1.30.0**.

Prima: 16 settembre 2026 (notte) — 🟡 **In `main` dopo 1.28.0, NON in pacchetto, nessuna migrazione**: **§A47** la **copia del database** si scarica da Diagnostica (solo Admin): `.sql.gz` scritto dal sito da una fotografia coerente, riga di chiusura con sha256, `tools/Vipi.DbBackup verifica`, due righe di audit; provata dal vivo su MariaDB 11.4.10 (ripristino col client `mariadb`, CHECKSUM identici) e messa nella CI (`andata-e-ritorno.sh`). Carta `docs/feature/2026-09-16-copia-del-database.md`. Con §A45 e §A46 fa una **1.28.1** (o 1.29.0 se si vuole segnare la funzione), da preparare quando il committente lo chiede. ▶ Online: provare il download col login e verificare il file con lo strumento.

Prima: 16 settembre 2026 (tarda sera) — 🟡 **In `main` dopo 1.28.0, NON in pacchetto, nessuna migrazione**: **§A45** la frequenza principale (★) è blu in tutte le tabelle — `AppFrequencies` (APP, vIPI ACC, vLOA, Live) aveva `freq-table` sulla riga invece che sulla tabella, misurato in produzione 0/1 contro 2/2 di aeroporto e militare (`b6544d86`, CI verde); **§A46** l'intro di `/services/vsop/mil` parte con le sezioni chiuse (`723e32a9`). ▶ Insieme sono una **1.28.1** di sole correzioni (Vipi.Ui.dll + `vipi-theme.css`), da preparare quando il committente lo chiede. ▶ **A42 da fare**: S11 shape, togliere il presidio della finestra (da confermare), feedback a due canali. 🔴 net10 = 1.29.0.

Prima: 16 settembre 2026 (sera) — ✅ **1.28.0 È ONLINE** (`docs/lavori-aperti.md` §A44), timbro confermato in barra e controlli da fuori verdi: pacchetto con la Ricerca, sei asset con sha256 identico, gli otto vSOP militari pubblicati con la tabella delle aree e nessuna scheda, aree su LIBV ed enhanced su LIBA tutti verdi. ▶ Col login: `Schema: 0`, elenco annidato dopo ricarico, «+ Sottosezione» su LIED. Il pacchetto: timbro **`1.28.0 · 4fd8ed7`**, zip `vipi-1.28.0-solo-file-cambiati.zip` sha256 `6758c162…cf166`, **30 file**, MINOR su 1.27.0, **nessuna migrazione**. Dentro §A39 (profondità 3→5, striscia del gesto perso), §A40 (aree di lavoro in una tabella), §A41 (elenchi annidati, campo che cresce), §A43 (banco regole col vento sulle piste in uso, ripiego senza frase italiana cablata). 🔴 **`Vipi.Infrastructure.dll` si spedisce col sorgente invariato**: `MaxDepth` è una `const` copiata dentro chi la usa (lezione aggiunta al runbook). Provato sul pacchetto: pacchetto 10/10, testo, aree militari, enhanced tutti verdi. ⚠️ Rifatto una volta: la foto ha trovato due difetti a schermo (stacchi nella cella del nome, conteggio «0 di 0») corretti in `4fd8ed7`. 🔴 Il net10 diventa **1.29.0**. ▶ **A42 da fare**: S11 shape, togliere il presidio della finestra (da confermare), feedback a due canali.

Prima: 16 settembre 2026 (pomeriggio) — 🟡 **In `main` e NON in pacchetto, nessuna migrazione**: **§A40** (`6ec7e04d`) nei vSOP militari le aree di lavoro e BOAT hanno **una tabella sola** — l'elenco a schede è spento (`RegulatedAreas.ShowCards=false`, solo MIL), attivazione e descrizione stanno in una riga che si apre con ▸, e le chip della mappa ora filtrano le **righe**. **§A41** (`ba01c664`) elenchi **annidati fino a cinque livelli** coi trattini (`- / --`, `1) / -1)`), mescolabili; sintassi in un posto solo (`VoceDiElenco`); in traduzione i marcatori **non partono**; tasti ⇤ ⇥, Tab/Maiusc+Tab e Invio nell'editor; i campi `data-adatta` crescono col testo fino al 60% dello schermo. 🔴 La CI di §A40 era **rossa** per `tests/conteggi-attesi.txt` non riscritto: sistemato in §A41. ▶ **Da fare, aperti dalla fine della finestra cieca (§A42)**: S11 della shape dei settori (§4-bis della carta 15, da ripianificare), togliere `MigrazioniDellaFinestraCiecaTests` (da confermare), feedback a due canali. ▶ Verifica dal vivo di §A40/§A41 non fatta.

Prima: 16 settembre 2026 (mattina) — 🟡 **In `main` e NON in pacchetto** (`0635fdd3`, CI verde, nessuna migrazione): **§A39**, due difetti **dal campo** sul SOD di Decimomannu — *«se creo una sottosezione, non appare nemmeno; sembra saturo»*. Nessuna delle tre ipotesi del segnalante (dimensione, database, host) era giusta; sotto quel sintomo c'erano **due guasti distinti**. **(1)** `DocumentSection.MaxDepth` si conta da zero ed era **3**, col catalogo militare appoggiato al bordo (`…› Procedure di partenza › VFR` sta esatto a 3): il motore rifiutava bene, ma il tasto «+ Sottosezione» restava **acceso** e il rifiuto si disegna nel callout **in cima alla pagina** — a tre schermate dal fondo non lo vedi. `MaxDepth` → **5**, tasto spento sul fondo **col motivo**. ⚠️ La **resa** però non cresce: dal livello 2 in giù gli stili sono gli stessi. 🔴 Quattro test pinnavano il **numero** e non la regola: riscritti. **(2)** 🔴 `avvii.txt` dice che Passenger spegne il processo entro **~10 secondi dall'ultima richiesta** (non «dopo qualche minuto»): il circuito muore di continuo e il gesto appena fatto **non arriva mai**, muto — ora dopo la ricarica compare «L'ultimo comando potrebbe non essere arrivato». ▶ **Al committente**: `passenger_min_instances` + idle timeout; `COLPETTO_MS` (150 s su una finestra di 10) si rivede **insieme** a quelli. **(3)** Le immagini non c'entravano, ma esiste un tetto per-documento — `MediaOptions.MaxBytesPerDocument` = **25 MB**, solo immagini, e quando scatta lo dice. Carta: `docs/feature/2026-09-16-sottosezione-al-fondo-e-gesti-persi.md`. ▶ Il committente sistema **due cose** prima: il prossimo pacchetto è **unico**.

Prima: 16 settembre 2026 — ✅ **1.27.0 È ONLINE** (`docs/lavori-aperti.md` §A38). Timbro **`1.27.0 · f6cbea3`**, zip `da9e6a1c…`, **28 file**, MINOR su 1.26.1 con **TRE migrazioni additive**. Da fuori, soli GET anonimi: pacchetto **8/8**, vAWOS **15/15**, navigazione enhanced **6/6**, e i quattro asset di `wwwroot` con **sha256 identico** a quelli spediti. Misurato in produzione: LIMC due blocchi e LIRF **tre**, P2000 dove il METAR non ha RVR, frecce nel senso di marcia, riga «RWY IN USE» **senza** provenienza da anonimo (e `dettaglio` null nel JSON), LIRJ col METAR di **LIRS** (la colonna nuova c'è: migrazioni entrate), scheda THE EYE nell'hub. ℹ️ `/vsop/health` = Degraded: severe dei **dati**, come in 1.26.1. ✅ **Timbro confermato in barra dal committente**; ▶ col login resta la riga `Schema: 0`. ▶ **Dati**: 🟡 gli APP non remotizzati si ripubblicano con calma (LIBA_APP, LICT_APP, LIRP_APP hanno ancora il vecchio indice, ed è previsto). 🟡 **THE EYE**: chiesto a chi lo sviluppa centro/zoom nell'indirizzo, in attesa. ▶ Far riprovare il bug del suggerimento SID. ▶ Il net10 (`l13-net10`, da riallineare a main) sarà **1.28.0**.

Prima: 16 settembre 2026 (notte) — 📦 **1.27.0 PRONTO DA CARICARE** (`docs/lavori-aperti.md` §A38): timbro **`1.27.0 · f6cbea3`**, zip `da9e6a1c…ef67b5`, **28 file**, MINOR su 1.26.1 con **TRE migrazioni additive** — 🔴 porta `Vipi.Infrastructure.MySqlMigrations.dll`, prova col login `Schema: 0`. Contiene §A33…§A37: SID nascoste/corrette, APP «Gestione del traffico»/«Tecnica operativa», sotto-sezioni fra i blocchi, stazione METAR di riferimento, **vAWOS a 2-3 piste** (freccia nel senso di marcia, testate arancioni, P2000, vento con piccole variazioni, provenienza della pista solo allo staff, regole e LVP dalla release), banco regole con tailwind/vento traverso per pista, THE EYE nell'hub. Provato sul pacchetto: pacchetto 10/10, vAWOS 15/15 (staff e pubblico), enhanced 6/6. ⚠️ Il net10 (`l13-net10`) diventa **1.28.0**.

Prima: 15 settembre 2026 (sera) — 🟡 **In main e NON in pacchetto, CON TRE MIGRAZIONI ADDITIVE** (`SidNascosteECorrette`, `SottosezioniFraIBlocchi`, `StazioneMeteoDiRiferimento` → il pacchetto porta `MySqlMigrations.dll`, prova `Schema: 0`). Dopo §A33 (`docs/lavori-aperti.md`): **§A34** APP non remotizzato — «Gestione del traffico» (IFR, VFR a blocchi) sopra i Coordinamenti e «Tecnica operativa» sotto (riconciliazione all'avvio); tendina «Collega frequenza» non più chiusa dentro la sezione; **§A35** sotto-sezioni e blocchi alternati nel corpo di ogni sezione (`CorpoDiSezione`, frecce unificate, via «Prima/Dopo il contenuto»); **§A36** stazione METAR/TAF di riferimento sull'anagrafica dello scalo (LIRJ, decoratore del provider meteo). CI verde fino a `146427dc`.

Prima: 15 settembre 2026 — 🟡 **In main e NON in pacchetto, CON UNA MIGRAZIONE ADDITIVA** (`docs/lavori-aperti.md` §A33): SID — «GOLF» di lied.sid e le SID commentate non entrano più come SID; SID **nascoste** al pubblico (`IsHidden`); **punto e transition corretti a mano** sulle importate (`FixOverride`/`TransitionOverride`, colonne a parte); `change` subito alla scelta di un suggerimento da datalist (bug NON riprodotto, da far riprovare); `conta-test.sh` ricuce le righe spezzate. Migrazione `SidNascosteECorrette` → il pacchetto porta `MySqlMigrations.dll`, prova `Schema: 0`. ⚠️ `l13-net10` va riallineato a main. CI verde `812d3acc`.

Prima: 14 settembre 2026 — ✅ **1.26.1 È ONLINE** (caricato il 14-set; da fuori pacchetto 8/8, vAWOS 15/15, navigazione enhanced 6/6; il `Degraded` di `/vsop/health` è un severe dei **dati**, confermato dal committente) (`docs/lavori-aperti.md` §A32): timbro **`1.26.1 · fae666e`**, zip `2fcdf65f…6e197a`, **40 file** (L2, L5…L12; nessuna migrazione; otto JS di `wwwroot` con `endpoints.json`). Provato sul publish: pacchetto 10/10, vAWOS 15/15, navigazione enhanced 6/6. 🔴 In produzione il runtime .NET è **8.0.28** (tre patch indietro): lo sistema **1.27.0**. ▶ **L13 fatto sul ramo `l13-net10`** (host net10, EF 8 + Pomelo invariati, SDK 10.0.401; ADR-0007 §D4-quater; CI verde), **non fuso**: si fonde e diventa 1.27.0 (carico completo) dopo che 1.26.1 ha retto qualche giorno.

Prima: 13 settembre 2026 (sera) — ✅ **La CI di `main` è VERDE** (`cd71c4d8`, quattro job), dopo **oltre 200 corse rosse** che nessuno guardava. Le cause, tutte invisibili in locale: il runner Linux gira in cultura invariante e leggeva il resx **italiano** (ora `tests/CulturaDellaSuite.cs` fissa en-GB); Brotli contro gzip su Linux; due test bUnit fuori dal dispatcher; tool degli asset net8 nell'immagine `sdk:10`; container non-root (T-062) col database in `/app` non scrivibile; `.dockerignore` che portava nel contesto i backup del database (oltre 6 GB, ha riempito C:); e **conta-test (T-056) che in CI non aveva mai contato**, perché con `--verbosity normal` il riepilogo non nomina l'assieme. Registro: T-056 e T-062 in `docs/history/audit-2026-09-13-revisione-totale-2.md`.
▶ **In main e NON in pacchetto** dopo 1.26.0: i lotti **L2, L5, L6, L7, L8, L9, L10, L11, L12** della revisione totale 2, nessuna migrazione → **1.26.1** quando il committente lo chiede. ⚠️ L12 cambia il JS (`vipi-editor.js`, `vipi-tour.js`): il pacchetto porta wwwroot. ▶ **Prossimo lavoro: L13** (piattaforma, T-059), e fuori lotto T-020 (IP dietro il proxy, prima si misura) e il cursore di T-055. ▶ Restano col login: `Schema: 0`, chiave al validatore dei tour e `Api__RichiediChiave=true`.

Prima: 13 settembre 2026 (pomeriggio) — ✅ **1.26.0 È ONLINE** (timbro confermato in barra; da fuori `pacchetto-verifica` tutto verde, archivio 200 senza chiave / 401 con chiave inventata, ACC inventato 404, pagina chiavi riservata). ▶ Col login: `Schema: 0`; poi chiave al validatore dei tour e `Api__RichiediChiave=true`. Il pacchetto: **1.26.0, cumulativa su 1.25.2, con DUE migrazioni additive** (contiene 1.25.4 e 1.25.3; `docs/lavori-aperti.md` §A30). Timbro **`1.26.0 · cad6698`**, zip `0b50dedc86bbcfd31ce793a5a0df5ae40beae22bb56c28f90f5a3b15f7a7741d`, 15 file — 🔴 da non dimenticare `Vipi.Infrastructure.MySqlMigrations.dll`. Porta T-017 (chiavi API: le API non sono mai anonime, `Api:RichiediChiave` ancora spento), T-042, L3, T-053, T-064/T-078. Provata sul pacchetto: migrazioni applicate, `pacchetto-verifica` e `chiavi-verifica` tutto verde. ▶ Dopo il carico: `Schema: 0`, i `curl` del foglio, poi chiave al validatore dei tour e `Api__RichiediChiave=true`.

Prima: 🟡 **1.25.4 PRONTA DA CARICARE, cumulativa su 1.25.2** (contiene 1.25.3; `docs/lavori-aperti.md` §A29). Timbro **`1.25.4 · d645c77`**, zip `6299d603…`, 11 file. T-002 (posizioni staff rilette da IVAO), T-011 (cache e lingua), T-084, T-085, T-061, T-019, T-021, T-033. ✅ Dopo il carico: `passenger_min_instances` e nginx; Cache Rule con la condizione sul cookie di lingua.

Prima: 🟡 **1.25.3 PRONTA DA CARICARE, ed è di SICUREZZA** (`docs/lavori-aperti.md` §A28). Revisione totale, secondo giro: 87 findings in `docs/history/audit-2026-09-13-revisione-totale-2.md`. 1.25.3 chiude l'S1 T-001 e T-003 (XSS) e porta T-012 (eliminare solo da Admin). Timbro **`1.25.3 · e3092ea`**, zip `5fac0b8246ea0b2830096c39916d29dbed275573f40e5c3643557d136d9650f0`, 7 file. 🔴 Cache Rule e `passenger_min_instances` **aspettano T-011** (la cache anonima non varia sulla lingua del browser).

Prima: ✅ **1.25.2 È CARICATA E IN PRODUZIONE.** Da fuori: `pacchetto-verifica` tutto verde, e **Q4 si esprime** — `blazor.web.js` ora `HIT` dal bordo (86-95 ms, prima 135-178 dall'origine), l'intestazione è arrivata a Cloudflare con la prima rivalidazione. Il pacchetto: Timbro **`1.25.2 · a6367d4`**, zip `803453c9f1ea57b2a77b36144d8f0fd0b40c4882fb4088bec8f8568701c4cf40`, **2 file** (`Vipi.Host.dll` e `.pdb`), foglio [`deploy/atc-ivao/LEGGIMI-PACCHETTO-1.25.2.md`](deploy/atc-ivao/LEGGIMI-PACCHETTO-1.25.2.md). PATCH: l'intestazione di cache di `blazor.web.js` ora si scrive **anche sul 304** — in 1.25.1 era online e inerte, perché Cloudflare rivalidava la copia vecchia e l'origine rispondeva 304. Provato sul pacchetto: timbro, `pacchetto-verifica` tutto verde, intestazione sul 200 e sul 304.

✅ **1.25.1 È IN PRODUZIONE** (12 settembre, notte) e i controlli da fuori sono in `docs/lavori-aperti.md` (13 settembre): Q1, Q2a, Q6, Q8 verdi; Q4 inerte (→ 1.25.2); 🔴 **Q5 non si esprime** — dodici richieste, dodici pagine ricostruite, mentre lo stesso binario in locale in Production serve corpi identici. La causa più probabile è §BG (processo fermato da fuori ogni ~46 s): **`passenger_min_instances` ora serve anche perché Q5 lavori**.

Prima: 12 settembre 2026 (notte) — 📦 il pacchetto 1.25.1, poi caricato. Timbro **`1.25.1 · e7d075b`**, sha256 dello zip `68632bf144a70ca2feec5490ccdd2425122989943d73ebe8a82e375319973de4`, **19 file**, 3,51 MB, foglio [`deploy/atc-ivao/LEGGIMI-PACCHETTO-1.25.1.md`](deploy/atc-ivao/LEGGIMI-PACCHETTO-1.25.1.md). **PATCH**: nessuna migrazione, nessuna pagina nuova, nessuna sezione, nessun resx — non c'è una funzione in più, ci sono le stesse funzioni che costano meno (le sette voci dell'audit prestazioni, §CZ). Provato **sul pacchetto**: `pacchetto-verifica.js` e `awos-verifica.js` **tutti e due TUTTO VERDE**, e lì si è chiuso il dubbio della sera — i due rossi «non minificato» erano l'artefatto del girare da sorgente. 🔴 **Ricostruito una volta**, e il difetto l'ha trovato la domanda «posso caricarlo?»: la chiave del gate di Q7 era di **41 caratteri** in una colonna `varchar(32)` che è una **chiave primaria**. In locale passava (SQLite non ha lunghezze); su MariaDB o fallisce o **tronca**, e troncata ogni 1.25.x sarebbe stata la stessa riga. Chiusa accorciando la chiave, usando il **commit** invece del numero (che si ripete) e con un test che misura anche le **chiavi composte**; controprova fatta. ⚠️ Al **primo** avvio dopo il caricamento le riconciliazioni rigirano e si timbrano solo se non hanno cambiato niente; poi si saltano (54 query) — ciclo provato per intero sul pacchetto. ▶ Dopo il carico: la **Ricerca**, il timbro col login, e `pacchetto-verifica.js` con `SOLO_PUBBLICO=1`. 🔴 E le **due cose del pannello** che valgono più di tutte e sette — Cache Rule su Cloudflare e direttive nginx — stanno nel foglio coi loro `curl`.
...
📦 **1.21.0 — il pacchetto.** Timbro `1.21.0 · 8291dd14`, **9 file**, foglio
...
annidare ancora.
> **16 settembre 2026**: il margine adesso c'è — `MaxDepth` è **5** (§A39). Chi ha provato ad annidare non ha
> letto il test: ha visto il tasto acceso e non succedere niente, perché il rifiuto si disegnava in cima a
> una pagina lunga tre schermate.
...
🟡 **E il debito è più largo di tre pagine: 26 pagine interattive, 74 iniezioni.** Misurato. Un presidio nuovo (`ScopeProprioDellePagineTests`) le elenca **per nome** e fallisce **solo sulle aggiunte**; c'è anche il rovescio, perché un elenco che nomina pagine sparite smette di misurare **restando verde**. Convertirle tutte tocca metà del prodotto: è una decisione, non un seguito ovvio.

⚠️ **Quel che serve indietro è UN FILE**: `diagnostica/errori-richieste.txt` fra qualche giorno. La corsa **non si riproduce qui** — in sviluppo il database è un file locale, in produzione è MySQL in rete — quindi la prova che sia sparita è vostra, non nostra. Nessun «A second operation…» = chiuse; su **altre** pagine = è una delle ventisei; sulle **stesse tre** = la cura non basta.

🔴 **E una lezione che vale oltre questo giro**: il primo giro di verifica su produzione aveva dato **tre rossi su tre** sul controllo della Ricerca — su un sito **sano**. Era la sonda: `window.Blazor` esiste appena lo script è interpretato, mentre il WebSocket può essere ancora in viaggio, e i tasti scritti in quella finestra non li raccoglie nessuno. Un falso rosso lì è la cosa più cara che quella sonda possa fare: è il controllo su cui si decide se **tornare indietro da una consegna sana**. Ora aspetta il primo fotogramma sul WebSocket di `/_blazor`.


**Ultimo aggiornamento:** 4 settembre 2026 — 📦 **IL PACCHETTO 1.7.0 È PRONTO E NON È ANCORA CARICATO** (`vipi-1.7.0-solo-file-cambiati.zip`, sha256 `88bf3a46…`, 4,43 MB, timbro **`1.7.0 · bf249155`**). **18 file**: cinque progetti coi loro `.pdb`, il satellite inglese, l'indice degli asset e **due CSS** coi compressi. In produzione gira ancora **1.6.2** (`84b0f4c7`). 🔴 **MINOR e non PATCH**, perché il numero segue il **contenuto**: §BD semina sezioni nuove nei documenti e §BE aggiunge un comando agli editor. **Nessuna migrazione**, quindi si consegna da sola via FTP anche dentro la finestra cieca. Cronaca completa in §BH.

**Che cosa porta.** §BB testata compatta · §BC parcheggi ai «Dati generali» · §BD «Carte aeroportuali» con cinque raccolte · **§BE le sezioni si muovono** (le libere cambiano gruppo, con la tendina «⇵ Sposta in…» e il trascinamento delle figlie nel menu-sezioni; dentro ci sono anche due difetti **vecchi**: «sopra il corpo» che sulle sezioni rese dalla pagina usciva sotto, e la sotto-sezione nascosta invisibile perfino in bozza sulla vIPI ACC) · §BF «Da un altro documento…» che **taceva** invece di dire che quel documento non ha righe · 🔴 **§BG la misura del keep-alive**.

🔴 **§BG è il motivo per cui questo pacchetto va caricato presto, e quel che chiediamo dopo è UN FILE.** Il processo in produzione muore **di età** — vita mediana **51 s**, 260 su 367 fra 45 e 60 secondi — e non di inattività: incrociando il secondo di avvio con quello di morte, la morte **segue l'avvio**. Perciò nessun ping può allungarla (fra ping a 30 s e a 10 s la mediana è passata da 46 a 51 s, cioè niente): il keep-alive **riaccende**, non tiene su. La domanda che decide la mossa dopo — **quei ping arrivano davvero al processo?** — da fuori non si vede, perché in tutti e due i casi chi bussa riceve un `200`. Ora la riga `ARRESTO` di `avvii.txt` lo dice: quante richieste ha servito, quanto fa l'ultima, chi l'ha svegliato, e se lo stop è arrivato da `SIGTERM` (fuori) o «**DA DENTRO**» (nostro codice). ⏳ **Serve `diagnostica/avvii.txt` riscaricato qualche ora dopo il caricamento**: `richieste 6/12/20…` ⇒ si guarda nel pannello dell'hosting; `richieste 1` o `0` ⇒ il keep-alive parla con qualcos'altro e va cambiato l'indirizzo.

⚠️ **Due cose da dire a chi carica**, e stanno nel foglio: `wwwroot` e l'indice degli asset viaggiano **insieme** (o le pagine escono senza stili — è il difetto del 24 agosto), e i documenti **già pubblicati** mostrano le correzioni di §BE/§BF **solo dopo una ripubblicazione**: una release è una fotografia e non si riscrive.

**Provato sul pacchetto, non sul sorgente**: suite intera in Release su **15 assiemi** verdi (E2E compresi), i **dieci** controlli di `pacchetto-verifica.js` sul publish win-x64 avviato dalla sua cartella, e le novità misurate a **1280 px** — tendina con 32 destinazioni dentro la card, riga dei comandi che non sfora, «Airport charts» già nell'indice (la riconciliazione ha lavorato al primo avvio). ⚠️ Il primo giro dava un rosso sul pannello traduzioni: era **lo strumento** — avviato in `Production` con l'autenticazione spenta si è anonimi. *Prima di accusare il pacchetto, guardare come si è misurato.*


**Ultimo aggiornamento:** 3 settembre 2026, sera — 📦 **IL PACCHETTO 1.6.2 È PRONTO E NON È ANCORA CARICATO** (`vipi-1.6.2-solo-file-cambiati.zip`, sha256 `28f0be88…`, 2,18 MB, timbro **`1.6.2 · 84b0f4c7`**). **Quattro file** — i soli `.dll`/`.pdb` di `Vipi.Host` e `Vipi.Ui`: il pacchetto più piccolo finora. Niente database, niente `wwwroot`, niente frasi. In produzione gira **1.6.1** (`6ffbe23a`), caricata la sera stessa. 🔴 **Il foglio di 1.6.2 CAMBIA una regola che sta su tutti i fogli precedenti**: fin qui dicevano «se compare `avvio-errore.txt`, fermatevi», e ora i file sono **due** — `arresto-errore.txt` **non ferma** il caricamento. Chi carica va avvisato, o si fermerà per niente. ℹ️ E il vecchio `avvio-errore.txt` rimasto sul server è il file che *mentiva*: va letto e **cancellato**, o darà un falso allarme a ogni consegna futura.

**§BA — due file per due guasti, e gli ultimi sette servizi fuori dal circuito.** Tutt'e due dalla **diagnostica di produzione**, non da una richiesta. (1) 🔴 `avvio-errore.txt` diceva **«l'avvio è FALLITO»** di uno **spegnimento**: `app.Run()` blocca fino alla chiusura, quindi il guasto dell'arresto usciva dal medesimo `catch` — e il processo incriminato era acceso da **1:50:57**, l'aveva detto `avvii.txt`. Non era 1.6.1 che non partiva: era 1.6.0 che finiva. Ora una sentinella su `ApplicationStarted` sceglie fra `avvio-errore.txt` (**fermatevi**) e `arresto-errore.txt` (**no**). ⚠️ Due file e non due intestazioni, perché il consiglio scritto sui fogli con un file solo resta **falso** per lo spegnimento più comune di questo hosting. (2) La guardia sugli `@inject` che toccano il database, nata guardando **un nome solo**, ha chiuso i **sette** casi tollerati: `AirportSectionsEditor` ×3, `NewDocumentPage`, `VersioniPage` ×3 — l'elenco dei tollerati è **vuoto**, con un `Assert.Empty` accanto perché a zero voci l'asserzione sarebbe vacua. ⚠️ `VersioniPage` era il caso da guardare, non il più semplice: accanto c'è la scelta **opposta** e documentata di `ReleasePanel` (che `IReleaseService` lo prende dal circuito *apposta*, perché il publish è composto col `BeforePublishAsync` della pagina ospite). Qui non vale: quella pagina **non monta `ReleasePanel`** — verificato col `grep`, non dedotto.

🔴 **E un buco trovato mentre si preparava il pacchetto, non dopo.** I test scritti col difetto provavano la **decisione** (`FileDelGuasto`, `Descrivi`) e non che qualcuno la **informi**: togliendo da `VipiStartup` la riga `ApplicationStarted.Register(...)` restavano **tutti verdi** e il comportamento tornava esattamente quello del difetto. È la famiglia del `[Parameter]` dichiarato e mai letto — nessun segnale. Chiuso con `SentinellaDellAvvioTests`, che avvia l'host vero, e provato **per mutazione**.

**Provato a schermo, e la prova sta nel DATABASE.** Ogni servizio spostato è stato *fatto scrivere* guidando l'app su una copia del `vipi.db` reale, e la scrittura verificata in archivio: `DocReleases` #57 (release al ciclo 2610), `AirportRunwayRules` #7, `AirportSectors` #7 `IsHidden=1`, `Documents` #33 (`vSOP MIL — LIBV`, campo solo militare che non aveva nessun documento). ⚠️ **Non provato, e detto anche nel foglio**: `IMilitaryDocumentService` *dentro* `AirportSectionsEditor` (il tasto compare solo su un campo misto senza vSOP, e in sviluppo non ce n'è), e `arresto-errore.txt` **non è stato visto nascere** — servirebbe uno spegnimento che va storto, e non si fabbrica a comando. ⚠️ Trappola dello strumento: la Ricerca ha dato «0 results for LI» dove il giro prima ne dava 50, ed era una **mia** variabile d'ambiente (`Ivao__Enabled=false`) — rilanciato senza, di nuovo 50. *Un risultato strano si spiega per differenza prima di chiamarlo difetto.*

ℹ️ **Ripulito anche `docs/lavori-aperti.md`**: **sei** righe affermavano al presente che §AO, §AR, §AS, §AT, §AY e §AZ non fossero in produzione, e lo erano tutte e sei (le prime da **1.4.0**, le ultime due da **1.6.0**). Verificato con `git merge-base --is-ancestor` contro il commit online, non a memoria. Le voci **datate** restano fotografie e non si toccano; una riga che afferma un fatto **al presente** è quella su cui qualcuno decide fra sei mesi.

**Ultimo aggiornamento:** 3 settembre 2026 — ✅ **1.5.0 È IN PRODUZIONE E LA DIAGNOSTICA È PULITA.** Timbro `1.5.0 · a071575`, Production, avvio alle 22:02:23 UTC; **nessun `avvio-errore.txt`**, **nessun `errori-richieste.txt`**, e in `avvii.txt` **ogni arresto è ordinato**. Otto controlli pubblici verdi + tre documenti veri col riquadro «Validità e revisione» a posto. 🟡 **Da tenere d'occhio**: l'avvio è passato da 6,5 s a **14,1 s** (le due voci di database, raddoppiate) senza che il codice faccia più lavoro — una misura sola per parte, serve una seconda `avvio-diagnostica.txt`. Dall'esterno: prima richiesta 18,2 s, poi 0,2 s. 🔴 **E resta la riga inglese** delle SID nella guida, corretta in `main` e **non ancora online**. Prima: ✅ 1.4.1 e ✅ 1.4.0. Vedi il riquadro sotto «Dove siamo».
...
> **5068 test**, di cui **276 E2E**. ⚠️ Il verde si legge **contando i progetti** nel riepilogo, non
...
> **tutt'e tre i riquadri di `admin/acc`** («ACC», «ACC sectors», «Regulated areas»), dove la maniglia del
> primo è **l'H1** (un titolo di sezione lì non c'è, era stato tolto perché ripeteva l'H1); ⓷ il piede e il
...
- Workflow **bozza→pubblicato** (clona versione, audit, archivia precedente). CRUD **blocchi e sezioni** (aggiungi/elimina/sposta, profondità fino a `DocumentSection.MaxDepth`: 3 fino al 16 settembre 2026, **5** da allora). `EditorPage` (`/{acc}/editor`, anche `?doc={id}`), `VersioniPage`.
