...
**Ultimo aggiornamento:** 18 settembre 2026 — ✅ **1.33.0 È ONLINE** (§A72 + §A69): ponte **RFO Gate Manager** `GET`/`PUT /api/rfo/events/{eventId}/state` per l'evento LIRN del 19 settembre (carta `docs/feature/2026-09-18-ponte-rfo-gate-manager.md`), migrazione additiva `PonteRfo`, timbro `1.33.0 · 7d25267`, zip `0651b7ca…`. Da fuori: sito 8/8, endpoint vivo, Cloudflare lascia passare. 🔴 **Chiave ancora rifiutata (401)**: il file in `segreti/` si legge SOLO all'avvio → il committente riavvia, poi curl + PUT paralleli su `prova-ponte-rfo` (mai sull'evento vero) e, col login, timbro e `Schema 0`. Poi si torna a §A70. net10 = **1.34.0**.
