...
**Ultimo aggiornamento:** 23 settembre 2026 — 🟡 **Aurora Sector Lab F3 in corso, slice 0-8 fatte** (§A116, carta [`docs/feature/2026-09-22-f3-l-app.md`](docs/feature/2026-09-22-f3-l-app.md) **approvata**, traccia slice per slice nel §8). `main` = `a3ab2397` + slice 8, **CI verde**, working tree pulito. Quattro progetti nuovi: `src/Vipi.SectorLab.Core` (logica), `src/Vipi.SectorLab.Ui` (pagine Blazor e il server locale), `src/Vipi.SectorLab` (il guscio Windows, `net10.0-windows`), `tests/Vipi.SectorLab.Tests` (213 test, girano in CI su Ubuntu). **Il sito non li referenzia e non cambia**; nessun dato di vIPI toccato, nessuna migrazione, niente pacchetto.
...
- ▶ **Prossima: slice 9, il salvataggio** (confini, conflitto, backup, scrittura atomica, rilettura) — la prima che scrive sul disco. Poi 10 problemi, 11 consegna.
