...
**Ultimo aggiornamento:** 17 settembre 2026 (tarda mattina) — 🟡 **§A59 in `main`, NON in pacchetto, nessuna migrazione**: il **registro del giorno** in `diagnostica/` — `richieste-AAAA-MM-GG.tsv` (una riga per richiesta: rotta dall'endpoint, esito, ms, pid, versione, autenticato; niente query, ping, statici) e `log-AAAA-MM-GG.txt` (righe `Vipi.*` da Information), sette giorni, tetto 5 MB per file; si leggono con `python tools/registro-del-giorno.py`. Carta `docs/feature/2026-09-17-registro-del-giorno.md`. Provato dal vivo. 🔴 `/vsop/live/atc` da 50 s NON è lentezza: è lo stream SSE. ▶ Pacchetto quando chiesto: `Vipi.Host` dll+pdb.
