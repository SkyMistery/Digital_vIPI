...
**Ultimo aggiornamento:** 22 settembre 2026 (notte) — 🟡 **Aurora Sector Lab F3 in corso, slice 0-7 e 7-bis fatte** (§A116, carta [`docs/feature/2026-09-22-f3-l-app.md`](docs/feature/2026-09-22-f3-l-app.md) **approvata**, traccia slice per slice nel §8). `main` = `01feffb6` + slice 7-bis, **CI verde**, working tree pulito. Quattro progetti nuovi: `src/Vipi.SectorLab.Core` (logica), `src/Vipi.SectorLab.Ui` (pagine Blazor e il server locale), `src/Vipi.SectorLab` (il guscio Windows, `net10.0-windows`), `tests/Vipi.SectorLab.Tests` (196 test, girano in CI su Ubuntu). **Il sito non li referenzia e non cambia**; nessun dato di vIPI toccato, nessuna migrazione, niente pacchetto.
...
- ▶ **Prossima: slice 8, aggiungi e togli un record**. Poi 9 salvataggio, 10 problemi, 11 consegna.
