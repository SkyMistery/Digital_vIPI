...
public static class GuideSearchCatalog
{
    ...
    public static readonly IReadOnlyList<Entry> Entries = new[]
    {
        ...
        new Entry("intro-di-pagina", "L'intro in cima a un elenco", "The intro at the top of a list",
            "intro introduzione testata elenco pagina sezioni libere pdf allegati biblioteca militari intro introduction header list page free sections attachments library",
            "La zona di sezioni libere in cima a un elenco: presentazione e PDF della biblioteca. Non e' un documento: quel che salvi e' subito pubblico.",
            "The free-form sections at the top of a list: an introduction and PDFs from the library. It is not a document: what you save goes live at once."),
        new Entry("editor-salva", "Indice laterale e salvataggio", "Side index and saving",
            "salva salvataggio salvato automatico gesto toc indice laterale bozza lock modifica save saving saved automatic toc index draft edit",
            "L'indice laterale, e il salvataggio nella bozza che avviene a ogni gesto.",
            "The side index, and saving into the draft, which happens on every change."),
        ...
        new Entry("admin-radioassistenze", "Radioassistenze: l'anagrafica di divisione", "Navaids: the division-wide catalogue",
            "radioassistenza radioassistenze navaid navaids vor ndb tacan vortac vortacan ils dme canale channel frequenza frequenza appaiata coordinate sessagesimali anagrafica divisione sectorfile import importa senza tipo elimina citata sorgente banda vhf famiglia navaid catalogue division sectorfile import no type delete cited source band family",
            "La pagina Radioassistenze: l'anagrafica condivisa, il tipo che lo scrive una persona, l'import dal sectorfile e le due guardie del cestino.",
            "The Navaids page: the shared catalogue, the type a person has to write, the sectorfile import and the two guards on deletion."),
        new Entry("admin-allegati", "Allegati: la biblioteca dei PDF", "Attachments: the PDF library",
            "allegato allegati pdf biblioteca drive google file link slug loa lettera accordo circolare carta manuale documento esterno versione sostituire caricare scaricare deposito ambito tipo attachment attachments library drive file link slug loa circular chart manual external version replace upload storage scope kind",
            "La biblioteca allegati: i PDF sul Drive di divisione, citati dai documenti con uno slug che non cambia mai.",
            "The attachment library: PDFs on the division Drive, cited by documents through a slug that never changes."),
