...
public static class VipiModuleExtensions
{
    ...
    public static IHost MigrateVipiDatabase(this IHost host)
    {
        ...
        if (db.Database.ProviderName?.Contains("Npgsql", StringComparison.OrdinalIgnoreCase) == true)
        {
            ...
            ReconcilePostgresColumns(db);   // EnsureCreated non altera le tabelle esistenti → allinea le colonne nuove del modello
        }
        else
            db.Database.Migrate();
        return host;
    ...
    /// <summary>Allinea lo schema Postgres al modello quando si usa <c>EnsureCreated</c> (deploy Render+Neon): questo crea
    /// le tabelle mancanti ma NON aggiunge colonne a tabelle già esistenti, così ogni colonna nuova del modello manda il DB
    /// in drift (es. <c>42703 column ... does not exist</c>). Per ogni colonna del modello relazionale assente nella tabella
    /// reale emette un <c>ADD COLUMN IF NOT EXISTS</c> col tipo store del modello; le NOT NULL ricevono un default sicuro per
    /// non violare le righe esistenti, poi il default viene rimosso (fedeltà al modello). Idempotente e best-effort: un
    /// errore su una colonna non blocca l'avvio. Sostituisce gli ALTER manuali per-colonna. Vedi ADR-0007.</summary>
    private static void ReconcilePostgresColumns(VipiDbContext db)
    {
        try
        {
            ...
            try
            {
                // Colonne reali dello schema public: chiave "tabellacolonna" (case-sensitive come le crea EF).
...
#pragma warning restore EF1002
        ...
        {
            Console.Error.WriteLine($"[vIPI] reconcile schema Postgres fallito (avvio prosegue): {ex.Message}");
        }
    }
