...
/// blocco.
/// <para>Il ricevente non è più fra le cose che si propagano, e non è una svista: è il lato B dell'accordo,
/// quindi non può divergere fra sorelle nemmeno volendo.</para>
/// </summary>
...
/// <param name="Count">Quante clausole ha il gruppo nell'accordo (non nella vista): è il numero che si annuncia.</param>
...
/// <summary>
/// Una clausola così come la tabella la deve rendere: il dato, l'accordo da cui viene, il posto che occupa nel
/// suo blocco di varianti, e le due frasi già composte.
/// <para>Il calcolo sta nella <b>pagina</b> e non nel componente di proposito: la vista ad accordo e la vista a
/// elenco costruiscono l'elenco in modo diverso (una scorre un accordo, l'altra li attraversa tutti), ma
/// producono la stessa cosa — e da lì in giù il codice che disegna è uno solo.</para>
/// </summary>
/// <param name="Agreement">L'accordo a cui la clausola appartiene: serve alle colonne di contesto in elenco.</param>
...
public static class XferLabels
{
    ...
    /// <summary>Gli enti di un lato dell'accordo, come si leggono in una riga sola. Vuoto = nessuno, che per il
    /// lato B significa «a UNICOM» e va detto, non lasciato in bianco.</summary>
    public static string Side(AgreementRow a, AgreementSide side)
    {
        var names = a.Parties.Where(p => p.Side == side).OrderBy(p => p.Order).Select(p => p.Callsign).ToList();
        return names.Count == 0 ? "" : string.Join(" · ", names);
    }

    /// <summary>Gli aeroporti dell'accordo in una riga sola («LIRF · LIRA · LIRU»); vuoto se non ne ha.</summary>
    public static string Airports(AgreementRow a) =>
