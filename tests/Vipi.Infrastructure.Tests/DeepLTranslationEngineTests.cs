...
public class DeepLTranslationEngineTests
{
    ...
    {
        var motore = new DeepLTranslationEngine(new StubFactory(new PrimaSiPoiNo()), Options.Create(new TranslationOptions
        {
            Enabled = true,
            DeepL = new DeepLOptions { ApiKey = "chiave-finta:fx", MaxTextsPerCall = 1 },
        }));

        var esito = await motore.TranslateAsync(new[] { "Uno", "Duecento" }, "it", "en");

        Assert.NotEqual(TranslationOutcome.Ok, esito.Outcome);
