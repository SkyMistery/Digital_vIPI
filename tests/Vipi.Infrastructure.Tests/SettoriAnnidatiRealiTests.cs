...
public class SettoriAnnidatiRealiTests : IAsyncLifetime
{
    ...
    private async Task<IReadOnlyList<SectorClaim>> Claims(params string[] online) =>
        SectorVolumeMap.BuildClaims(
            await new EfSectorVolumeCatalog(_db, new EfSectorShapeResolver(_db, new EfSectorAirspaceBindings(_db, LivelloFisso.Editor), new EfSectorShapeParts(_db))).GetAllAsync(),
