...
| `Rfo:Chiavi:<nome>:Chiave` / `…:Eventi` | string / string | *(nessuna)* | Chiavi del **ponte RFO Gate Manager** (`/api/rfo/events/{eventId}/state`), **nei segreti**, NON nella tabella `ApiClients`. `Eventi` = eventi separati da virgola, `*` = tutti, vuoto = nessuno (403). Chiave < 32 caratteri = ignorata. 🔴 I file di `segreti/` si leggono SOLO all'avvio: dopo averne messo uno, riavviare. Dizionario per nome e non array: il binder somma gli array. Header `x-api-key`. Carta `docs/feature/2026-09-18-ponte-rfo-gate-manager.md`. |
...
- `GET`/`PUT /api/rfo/events/{eventId}/state` — ponte fra le copie di «RFO Gate Manager» di un evento RFO, con chiave `Rfo:Chiavi`. Carta `docs/feature/2026-09-18-ponte-rfo-gate-manager.md`.
