...
#### 🔴 A18-bis — IL VERDETTO SUL DIFETTO NON È ANCORA DATO
...
- 🔴 **A18-bis**: rileggere `avvii.txt` dopo le 19:48 locali. È la voce che chiude (o riapre) §AM-A, e
  sostituisce §AM3 con una scadenza precisa invece di «fra qualche giorno».
- ❓ **`errori-richieste.txt`**: non era fra i file mandati, e le due letture sono **opposte** — o è stato
  cancellato (§A16/§AM4) e **non si è ricreato**, cioè zero richieste fallite da 1.2.0 in poi, oppure è
  ancora sul server e non è stato scaricato. Va chiesto: se esiste, è la seconda metà del verdetto.
- 🔴 **§AM2** resta aperta e **non si chiude da fuori**: le corse sul `DbContext` si vedono solo da
  **collegati**, su `/services/vsop/libb/editor`, perché è lì che si aprono con la latenza del database
  remoto. Il gesto: entrare col proprio VID, aprire l'editor di LIBB, salvare qualcosa. Se il pannello
  traduzioni non fa più comparire «A second operation was started», §AM-C è confermata in produzione.
- 🟡 **§AM4**: cancellare `errori-richieste.txt` sul server, se è ancora lì.
