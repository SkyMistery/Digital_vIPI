...
▶ **Restano due prove**: il timbro `1.21.1 · 034f9187` (barra da amministratore, o `avvio-diagnostica.txt`) e un `errori-richieste.txt` di giorno.
...
🔴 **E provare il PACCHETTO invece del sorgente ha pagato di nuovo, due volte nello stesso giro.** (1) A processo appena acceso il selettore di postazione diceva «Nessuna postazione» invece di «Caricamento…» — su Plesk, dove il processo rinasce ogni ~50 s, quel momento lo vede la **prima visita di ognuno**. (2) 🔴 E la prima stesura della correzione ha aggiunto un difetto **peggiore**: il commento che la spiegava stava **fra gli attributi** del componente, e un commento Razor scritto lì dentro diventa il **nome di un attributo** — build a **zero avvisi** e la vista live in **500 per tutto lo staff**. L'ha preso lo smoke `Live_page_renders`. ➕ **E i `packages.lock.json` hanno ballato di nuovo**: col solo `linux-x64` dichiarato li rivestiva il publish **win-x64**, che il runbook prescrive a ogni pacchetto. I RID dichiarati ora sono **due** — la regola di stamattina applicata a sé stessa, invece di ripulire a mano dopo la prova.

✅ **Provato sul pacchetto pubblicato**: dieci controlli su dieci verdi (JS minificato servito, circuito aperto, **Ricerca che risponde**, foglio di stile in vigore, console pulita), più il selettore e la ricerca SID guidati a schermo **a processo appena acceso**. Build Release `no-incremental` **0 avvisi**, **10.433 test** su quindici assiemi.

**Aggiornato:** 5 settembre 2026 — ✅ **§BR: LA VISTA RAPIDA AEROPORTO CERCA LE SID, E LA SUA TABELLA SMETTE DI ESSERE UNA SECONDA DERIVAZIONE.** Richiesta del committente: «un modo per cercare le SID in base al punto o al codice, e nella tabella anche transition e condition come nel viewer di aeroporto». 🔴 **La cosa importante non era la ricerca**: `AirportQuickPanel` partiva dalle `SidRow` grezze e rifaceva a mano filtro AIRAC, ordine e trattini — una **seconda derivazione** della stessa tabella, che divergeva da quella del documento completo. La nota «to coord with APP» sull'initial climb la aggiunge la derivazione, quindi lì **non compariva mai**: misurato dopo la cura, su LIBD sono **7 righe su 20**. Ora il pannello chiama `IAirportSidDerivationService` come il documento. ➕ **Transition e Condition ci sono sempre** (erano condizionali); **Cat./WTC restano condizionali ma si decidono su tutto lo scalo** — 🔴 legate alle righe filtrate, la tabella cambiava numero di colonne **mentre si digitava**. ➕ **Ricerca per punto, codice e transition** (la transition è un punto anche lei), predicato puro e testato in `AirportViewFormat.SidMatches`. 🔴 **E il campo ha dovuto cambiare forma**: con `value="@_q"` rimandato a ogni tasto i caratteri cancellati **ricomparivano** (otto Backspace, tre sopravvissuti, a ritmo umano) — niente `value` e `@key` per svuotarlo dal server. ✅ Verificato a schermo su LIRN/LIBD/LIBR/LIPA, a ritmo umano e a raffica, console pulita. ✅ **Fuso in `main`** (`bc684372`, ramo cancellato da locale e da `origin`), punto di ritorno `main-prima-del-merge-20260905-ricerca-sid`: build Release `no-incremental` **0 avvisi** e **5.438 test verdi sul merge**. ⚠️ Va nello stesso pacchetto di §BN e §BQ: `Vipi.Ui` (frasi e `wwwroot` compresi), nessuna migrazione. ℹ️ **Nessuna SID in archivio ha una Condition**: la colonna nuova esce tutta a «—» perché il dato è editoriale e non è compilato — anche il documento completo la mostra vuota. ⚠️ **E resta aperta la stessa forma in `TypeaheadPicker`** (`value=` + `@oninput`), il campo a digitazione usato da mezzo prodotto, selettore di postazione di §BQ compreso: cancellare in fretta lì dentro ha lo stesso difetto. Non toccato perché fuori dalla richiesta.

**Aggiornato:** 5 settembre 2026 — ✅ **§BQ: LA VISTA LIVE NON È PIÙ PUBBLICA, E CHI PUÒ SE LA SCEGLIE.** Richiesta del committente: «per chi è almeno division staff vorrei poter selezionare una qualsiasi postazione per vedere l'interfaccia come se fosse live quella postazione; per gli utenti normali non voglio la possibilità di selezionare la pagina di un ente aperto che non sono io». 🔴 **Il buco c'era ed era largo**: `/services/vsop/live/{callsign}` non aveva **nessun** cancello — bastava scrivere un callsign nell'indirizzo, da anonimo. Ora l'utente normale apre **solo la propria** postazione (qualunque altra = rimando silenzioso a `/services/vsop/live`, `replace:true`), e da **DivisionStaff** in su c'è un **selettore** su **tutte** le postazioni dei cataloghi, spente comprese — il modello si compone dai cataloghi, quindi «come se fosse live» era già quello che la pagina sapeva fare. ⚠️ **La regola è «la mia», non «non è aperta da altri»**: un cancello che guardasse l'online cambierebbe risposta a ogni tick SSE e butterebbe fuori chi sta guardando. ➕ Spenti anche i **tre gesti che rimbalzerebbero**: ATC online dello stato d'attesa, pastiglie della catena di copertura, tasto «Operativa» di `AppnPage`. Il selettore riusa `TypeaheadPicker` (niente settimo campo a digitazione) e legge l'elenco **una volta sola, sotto lo stesso semaforo** delle altre letture della pagina. ⚠️ **Cambia un principio scritto**: doc refactor 12 §3 e la memoria `live-view-design` dicevano «nessun selettore» — aggiornati nello stesso giro. Carta: `docs/feature/2026-09-05-vista-live-selettore-e-cancello.md`. ✅ **Fuso in `main`** (`06ee1e10`, ramo cancellato da locale e da `origin`), punto di ritorno `main-prima-del-merge-20260905-vista-live` (= `7a5007bd`): build Release `no-incremental` **0 avvisi** e **5.429 test verdi sul merge**. ✅ **Verificato live** a tre identità `DevIdentity` (User scollegato, User **connesso** LIEE_TWR, DivisionStaff `IT-T01`), e la verifica ha trovato **due** difetti che la suite non vedeva — l'ordine del selettore e la pastiglia verde fissa: §8 della carta. ✅ **ORA ONLINE con 1.10.0** (era: serve un pacchetto). Toccava `Vipi.Ui` (frasi comprese) e `Vipi.Application`; nessuna migrazione. ⚠️ Nel foglio è scritto che la vista live di un collega **non si apre più**: chi non è staff di divisione e ci arriva da un preferito viene rimandato alla propria, in silenzio.

**Aggiornato:** 5 settembre 2026 — ✅ **§BN È IN `main`** (fuso, ramo cancellato: build Release `no-incremental` e 5.423 test verdi sul merge). ➕ Due code dello stesso giorno, tutt'e due in `main`: **§BO — i `packages.lock.json` hanno smesso di ballare** (il RID si DICHIARA, e non era rumore: la CI restora in «locked mode»), e **§BP — la nota della quota da concordare è corta ovunque** «to coord with APP», in **una costante sola**, dopo che nell'editor scavalcava la colonna Cat. ✅ **ORA ONLINE con 1.10.0** (era: serve un pacchetto). ⚠️ Nel foglio è scritto che l'editor d'aeroporto **non ha più tasti «Salva»**, o chi lo apre penserà che sia rotto.

**Aggiornato:** 4 settembre 2026, notte fonda — ✅ **§BN: L'AEROPORTO HA UNA PORTA SOLA, E I CAMPI VIVONO SOLO COL LOCK.** Richiesta del committente: «vorrei che il punto di salvataggio dei documenti di aeroporto sia uno, come negli altri documenti, e voglio poter scrivere nei campi solo se il documento è in editing». Erano **nove** tasti «Salva» più «Salva tutto (N)» e Ctrl+S, **tre** buffer che si potevano perdere e un cancello che non era il lock ma il **ruolo** sulla ACC — si digitava in tutta la pagina senza aver premuto «Modifica». Ora ogni gesto scrive, e il lock è una guardia **nel service**, non un tasto spento. ✅ **Fuso in `main`** il 5 settembre (`8b3130ad`, ramo cancellato) e ✅ **ONLINE con 1.10.0**. Carta: `docs/feature/2026-09-04-aeroporto-porta-sola.md`.
...
**Aggiornato:** 4 settembre 2026, sera tardi — ✅ **§BL: LE TRE CORSE DELLA DIAGNOSTICA SONO CHIUSE**, in `main` e ✅ **online da 1.8.1**. Le corse non venivano da dentro un caricamento — quelli aspettano un `await` per volta, letto — ma da **due flussi che partono insieme** sullo stesso `DbContext` **di circuito**: `AeroportoEditorPage` e `MilEditorPage` leggono l'unione in `OnParametersSetAsync` mentre il componente figlio fa il suo `LoadAsyncCore` (08:15, 10:16, 12:06), e `MilListPage` carica mentre una query della pagina **precedente** è ancora in volo (07:22). 🔴 **La coda dell'editor non era la risposta, benché esista apposta**: `DocumentEditorShell.InFilaAsync` sta sul **componente**, e quando la pagina legge il riferimento al componente è ancora `null` — lo scrive il primo render, che deve ancora avvenire. La cura è quella già usata due volte: **`OwningComponentBase`**, cioè uno **scope proprio**, come §BJ per l'import tabelle. ⚠️ Serve anche `@using Microsoft.Extensions.DependencyInjection`, o `ScopedServices.GetRequiredService` non compila.

🟡 **E il debito è più largo di quelle tre pagine: 26 pagine interattive, 74 iniezioni.** Misurato, non stimato. Un presidio nuovo (`ScopeProprioDellePagineTests`) le elenca **per nome** e fallisce **solo sulle aggiunte**: convertirne una vuol dire toglierla dall'elenco, e il verde torna da sé. C'è anche il rovescio, che serve quanto l'altro — un elenco che nomina pagine sparite smette di misurare **restando verde**. ⚠️ **La lista dei servizi «sicuri» è corta e motivata**: `IEditAuthorizationService` ci sta perché dal 28 agosto il livello si risolve **senza una query**. 🔴 **Convertire le altre 26 non è stato chiesto**: tocca mezzo prodotto, ed è una decisione, non un seguito ovvio.

**Provato a schermo** su copia del DB: le tre pagine si aprono, il circuito regge, console pulita, e **quattro andirivieni** fra editor ed elenco — che è il gesto che provocava il caso delle 07:22 — senza un circuito caduto. Build Release 0 avvisi, 15 assemblee verdi. ⚠️ **Nessuna migrazione**, ma **non è in produzione**: 1.8.0 è salita alle 12:48 e questo è arrivato dopo. ✅ Il pacchetto **1.8.1** è stato preparato la sera stessa — tocca solo `Vipi.Ui`, quattro file — e resta da caricare.


**Aggiornato:** 4 settembre 2026, sera — ✅ **1.8.0 È IN PRODUZIONE** (caricata dal committente, avvio 12:48:11Z, timbro `1.8.0 · 8dc05f4`, ambiente Production, MySql, configurazione sana, **nessun** `avvio-errore.txt`). ✅ **Verificata da fuori, da anonimo, tre giri su tre verdi**: e i due punti di §BK visibili al pubblico si vedono davvero — su LIMC la legenda dice *«recommended. Wind 240° / 6 kt.»* e **non** nomina la regola, col vento nel carattere della tabella; e la colonna **WTC** sta su **una riga** su LIMC/LIBD/LIPZ (108px, 92 utili), con la **controprova** che alla larghezza di prima (57px) torna su due. ℹ️ **L'avvio è sceso da 15 103 ms a 6 309 ms** (migrazione 2 720 contro 9 808, manutenzioni 3 001 contro 3 857): un terzo del tempo di prima.

🔴 **MA IL PRIMO GIRO DI VERIFICA ERA USCITO ROSSO, E IL DIFETTO ERA DELLA SONDA.** «La Ricerca non risponde — sito mezzo caricato», tre volte su tre. Guidando la pagina a mano rispondeva: **50 risultati per «LI»**. ⚠️ `window.Blazor` **non vuol dire che il circuito sia aperto**: quell'oggetto esiste appena lo script è stato interpretato, mentre il WebSocket può essere ancora in viaggio — e i tasti scritti in quella finestra non li raccoglie nessuno. Misurato: senza attesa 3 rossi su 3, con 2 000 ms di attesa «50 results» tutte le volte. La sonda ora aspetta il **primo fotogramma sul WebSocket di `/_blazor`** (via CDP), che è quel che la sezione 4 del runbook dice da sempre e che **proprio in quel controllo** non si faceva; e confronta il testo **intero** invece dei primi 400 caratteri, taglio che reggeva per caso («50 results» cade al carattere 88). 🔴 **Un falso rosso lì è la cosa più cara che quella sonda possa fare**: è il controllo su cui si decide se tornare indietro da una consegna sana. Corretta e riprovata: **3 giri su 3 verdi**.

🔴 **E `errori-richieste.txt` porta una famiglia che 1.8.0 NON chiude.** Quindici voci, tutte `CircuitUnhandledException`, in tre gruppi da tre più le vecchie. ✅ Quelle dell'**import tabelle** (07:23, 07:36 — `RisolutoreCelle`) sono §BJ, e 1.8.0 le chiude. 🔴 Le altre **no**, e due gruppi sono su **1.7.1**, cioè su codice che 1.8.0 ha ancora: **08:15 e 10:16** — `AirportSectionsEditor.LoadAsyncCore` che corre contro `DocumentUnionService.ForDocumentAsync`; **12:06** — `MilSectionsEditor.LoadAsyncCore` contro `EfDocumentAdminRepository.ListAsync` e `EfEditingRepository.LoadForEditAsync` (quattro query aperte insieme sullo stesso DbContext). ⚠️ **La diagnosi la scrive la diagnostica stessa**, e il colpevole ha un nome: `AeroportoEditorPage` e `MilEditorPage` hanno `OnParametersSetAsync => LeggiUnioneAsync()`, cioè **la PAGINA legge il database fuori dalla coda dell'editor** (`DocumentEditorShell.InFilaAsync`) mentre il componente figlio carica. Due flussi, un solo DbContext di circuito. È una coda di §AZ (documenti uniti): `LeggiUnioneAsync` è nato con quella carta e non è mai entrato nella coda. ⚠️ **Da fare**, e non è stato fatto: nessuno l'ha chiesto ancora.

ℹ️ **Il keep-alive non cambia**, e ora il contatore lo dice per intero: 67 arresti con `richieste N`, **tutti** fermati da `SIGTERM`, vita **mediana 48 s** con **6 richieste** ciascuno, svegliati 65 volte su 67 da `/vsop/health/ready`. ⚠️ Le due eccezioni sono la prova del contrario: i due processi svegliati da `/_blazor/negotiate` e `/vsop/live/atc` — cioè da **persone vere** — sono vissuti **1h30** e **51 minuti** (381 richieste). I ping arrivano e non bastano; un circuito aperto sì. Resta il **pannello dell'hosting**, dopo il 16. ℹ️ La quiete fra le 10:54 e le 12:48 non è una cura: erano quei due processi lunghi.


**Aggiornato:** 4 settembre 2026, sera — 📦 **PACCHETTO 1.8.0 PRONTO** (`vipi-1.8.0-solo-file-cambiati.zip`, sha256 `4010d6c03e17a30e0a31414652eb32da043e0529b59a6f060cd7a4989d6de2e8`, **3,33 MB**, **11 file**). Timbro **`1.8.0 · 8dc05f4`**. 🟢 **Nessuna migrazione**: si consegna da sola via FTP anche dentro la finestra cieca. ⚠️ **MINOR e non PATCH**, per il CONTENUTO: la biblioteca allegati prende un asse di ricerca e le cartelle, il banco di prova delle regole piste prende un campo, e la vIPI d'aeroporto **mostra meno** a chi non è staff di divisione. ⚠️ **E questa volta c'è `wwwroot`**: `vipi-theme.css` è cambiato, quindi entrano il foglio, i suoi `.br`/`.gz` e **l'indice degli asset** — i quattro viaggiano insieme, ed è l'unica trappola vera del pacchetto (difetto del 24 agosto). Dentro ci sono **§BJ** (import tabelle con scope proprio) e **§BK** (le cinque rifiniture). ⚠️ Le impronte hanno tolto tre `.dll` che il diff per impronta proponeva: `Vipi.Infrastructure`, `Vipi.Hosting` e `MySqlMigrations` differiscono per il solo MVID, e nessuna firma pubblica di `Vipi.Application` è cambiata (solo aggiunte), quindi nessun assieme rimasto indietro può alzare `MissingMethodException`. **Provato sul pacchetto pubblicato**: dieci controlli verdi del driver di pacchetto, il foglio di stile provato è **lo stesso file** che si spedisce (impronta confrontata), e i cinque punti guidati a schermo **con due identità** — da amministratore la legenda dice «dalla regola attiva «#1»», da sotto-staff dice solo «consigliati» col vento. WTC misurata: chiede 56px, la colonna nuova ne offre 91, e la **controprova** alla larghezza di prima la rimanda a capo. Le 11 impronte **riverificate dentro lo zip** dopo averlo costruito. Foglio: `deploy/atc-ivao/LEGGIMI-PACCHETTO-1.8.0.md`. 🔴 Resta da **caricare via FTP**.


**Aggiornato:** 4 settembre 2026, sera — ✅ **§BK: CINQUE RIFINITURE DELL'AEROPORTO E DEGLI ALLEGATI**, ramo `editor-aeroporto-rifiniture`, cinque commit. Richiesta del committente, cinque punti, tutti chiusi. 🔴 **Il punto che vale più di tutti non era nella richiesta**: leggendo il motore per aggiungere il momento al banco di prova è saltato fuori che **una regola pista che scavalca la mezzanotte si spegneva a mezzanotte**. «Lunedì 22:00–05:00»: alle 03:00 di martedì il filtro guardava il giorno di **calendario**, la maschera diceva lunedì, e la regola smetteva di applicarsi **a metà della notte che descrive**. Ora c'è il **giorno operativo** (`RunwaySuggestion.GiornoOperativo`), e lo scalino vale per **tutti e tre** i filtri di calendario — giorno della settimana, parità, stagione — o «le notti dei giorni dispari» si spezzerebbe allo stesso modo. ⚠️ **Cambia il comportamento di regole già pubblicate**, ma solo di quelle con finestra che scavalca; senza scavalco non cambia niente. Quattro prove, e senza la correzione ne cadono tre. **(1) Banco di prova**: un campo `datetime-local` solo — da un istante si ricavano giorno, ora, parità e stagione, tutti coerenti fra loro, mentre quattro campi separati si possono contraddire — più «Adesso», il giorno scritto per esteso, e il gettone **«giorno dopo»** accanto all'orario di fine quando la finestra scavalca. La conversione di fuso la fa il **motore** (`OraLocale`/`AUtc`), non l'editor. **(2) La meccanica delle regole scende a chi le scrive**: «recommended by the active rule «#2»», la nota della regola e il segno «adesso» nella tabella delle regole vogliono **DivisionStaff**; il cancello è un parametro del componente che nasce **chiuso**. La tabella delle regole resta, è documento pubblicato. Nello stesso giro il **vento** passa al carattere della tabella (13px, inchiostro pieno, peso del numero di pista): è il dato che si cerca, non una didascalia. ⚠️ La pagina è SSR **statica e senza cache d'uscita**, quindi il livello si può chiedere per richiesta — se un giorno finisse dietro una cache il cancello andrebbe rifatto. **(3) WTC del viewer da 5,5% a 10,5%**, i cinque punti li ridà Condition: il minimo era misurato sul dato tipico («M», «M,H») e non sul **caso pieno**, dove «L,M,H,S» andava a capo **anche a 1920** — lì la colonna centrale è ancora sotto il `min-width` della tabella, quindi le percentuali si calcolano su quello e non sulla finestra. **(4) I chip del filtro SID lasciano cadere la selezione**: la barra agisce sulle righe **scelte**, e dopo un cambio di pista quelle righe non sono più a schermo — si pubblicava al buio. La ricerca no (si scrive a un carattere per volta), le righe **toccate e non salvate** neanche. **(5) Biblioteca allegati, cartelle RICAVATE**: a **121 voci** in produzione mancava l'asse che conta, la **chiave del perimetro** — «gli allegati di un ACC» si chiedeva, «quelli di Milano» no, e gli ACC sono quattro. Terzo asse con un chip per chiave presente (contati **senza sé stessi**, o non si potrebbe più cambiare la scelta), elenco **ordinato** per perimetro/tipo/titolo, righe raccolte per perimetro con testata richiudibile e «apri/chiudi tutto». ⚠️ **Niente cartelle in banca dati**: la carta della biblioteca le rifiuta perché un albero lo si riempie a mano e ci si archivia male; queste si ricavano dai campi che ci sono già, quindi nessuno le può sbagliare perché nessuno le sceglie — **e nessuna migrazione**, che nella finestra cieca fino al 16 settembre non è un dettaglio. Filtro, ordine e raggruppamento in `AttachmentBrowsing`, cuore deterministico. **Nuove prove: 4 sul motore, 3 sul componente piste, 2 sull'editor SID, 8 su `AttachmentBrowsing`, 4 sulla pagina allegati.** Le prove che c'erano contavano `tbody tr` e ora contano le righe **di dato** (le testate dei perimetri sono righe di tabella anche loro). Build Release **0 avvisi**, **14 assemblee verdi**. ⚠️ Nessuna migrazione; ✅ **online da 1.8.0** (toccava `vipi-theme.css`, quindi anche l'indice degli asset). ✅ **VERIFICA LIVE FATTA**, su copia del DB, con **due identità**: admin (VID 704798) e sotto-staff (`DevIdentity__UserId=123456`, posizione `XX-ZZ9`). 🔴 **E ha trovato un difetto che i test non vedevano**: il punto in coda alla legenda delle piste era scritto `<text>.</text>` **fuori** da un blocco di codice, dove `<text>` non è un comando di Razor ma un **elemento HTML sconosciuto** — finiva nel DOM alla lettera, e a schermo si leggeva giusto perché il browser lo rende inline. Visto nel **prerender** di LIBD, non dai test; ora la prova asserisce che nel markup non ci sia nessun `<text>` e senza la correzione cade. **Che cosa si è guardato**: legenda admin «consigliati dalla **regola attiva «#1»**» → da sotto-staff «consigliati.» e basta, col **vento sempre presente** e misurato **identico** alla tabella (13px, `rgb(250,250,255)`, peso 600 contro il `<b>` della cella); WTC «L, M, H, S» — ⚠️ **con gli spazi**, che chiede 56px e non 40 — in **una** riga a 91px utili, e la **controprova** rimettendo il 5,5% di prima lo fa tornare **a capo su due righe** a 40px utili; banco di prova col campo data/ora popolato e il gettone **«giorno dopo»** che compare scrivendo 22:00→05:00; chip pista dell'editor SID con **39 righe scelte → 0**; biblioteca raccolta per perimetro e testata che richiude. ⚠️ **Una cosa NON si è potuta guardare**: il segno «adesso» nella tabella delle regole, perché quella tabella viene dallo **snapshot congelato** della release e nessun aeroporto del DB di sviluppo ha regole pubblicate — la regola seminata per la prova si vede nella legenda (che è live) ma non nella tabella. Il cancello è lo **stesso** `MeccanicaVisibile` sulla riga sopra, e che `ActivePosition=null` non disegni il segno lo tengono le prove che c'erano già. ⚠️ Nel dev DB la biblioteca ha **una** voce sola, quindi il raggruppamento a schermo si è visto su un perimetro solo: il caso a centoventuno voci lo tengono le prove di `AttachmentBrowsing`.

**Aggiornato:** 4 settembre 2026 (sera) - ✅ **1.7.1 È IN PRODUZIONE** dalle 10:03:04Z (timbro `1.7.1 · f4d7347`, avvio 15 103 ms, configurazione sana). 🔴 **E la misura di §BG ha dato il VERDETTO sul keep-alive: i ping ARRIVANO al processo e non bastano.** 33 processi corti misurati, vita media **46 s**, **5-7 richieste ciascuno** (una ogni 7,8 s, cioè esattamente i sei ping del Worker), svegliati da `/vsop/health/ready` e fermati **sempre da SIGTERM**, con l'ultima richiesta arrivata **0-9 s prima di morire**: non è mai inattività, lo uccide qualcuno da fuori. ⚠️ **E un processo è vissuto 1h 29m con 609 richieste** - l'unico del file `svegliato da /_blazor/negotiate`, cioè da un browser vero con un circuito aperto: **non esiste nessun tetto d'età**, e nessuna frequenza di ping potrà mai bastare, perché una connessione lunga tiene su il processo e una richiesta corta no. **→ La strada è il pannello dell'hosting** (`passenger_min_instances`, tempo di inattività del pool). 🔴 **Decisione del committente: si chiede DOPO IL 16 SETTEMBRE**, perché chi può metterci mano in Ivao.It è via — la stessa assenza della finestra cieca. ⚠️ Fino ad allora **il sito continua a rinascere ogni ~50 s ed è atteso**: non è un difetto nuovo da indagare. E **§BJ** è in `main` ma 🔴 **non si spedisce da solo: entra nel prossimo giro**, quando ci sarà altro da consegnare.
...
**§BE, seguito della stessa sera** — il comando c'era e **non si poteva premere**: a 1280 px la riga dei comandi sforava di 68 px (197 su una sotto-sezione) e il menu in linea usciva dalla card, con tutte e trenta le destinazioni fuori. ⚠️ Avevo copiato il menu di «+ Blocco» **col suo commento**, ma quel commento vale nel **corpo** della sezione, che va a capo. Ora è una **tendina** di sistema, e la riga dei comandi va a capo **solo in modifica**.
...
📦 **Spedita nel pacchetto 1.7.0** (§BH), pronto il 4 settembre e non ancora caricato.
...
📦 **Spedita nel pacchetto 1.7.0** (§BH). Cinque test bUnit (`ImportDaAltroDocumentoTests`), provati per
mutazione: rimesso il componente di prima, tre diventano rossi. Guidato a schermo su copia del DB (LIBG, sezione Nominativi): con una sorgente
...
### BM9 ✅ Il pacchetto 1.9.0 — IN PRODUZIONE, e le due cose che la prova ha corretto

**15 file**, timbro `1.9.0 · f75e2e0`, sha256 dello zip
`3045b044acf77bdd2deefd2c1ed911e2d2a2ec6cce3995cfffede0c9143d73ee`.

Dentro: i quattro assiemi cambiati (`Domain`, `Application`, `Infrastructure`, `Ui`) coi loro `.pdb`, il
satellite inglese, `Vipi.Host.dll` (il **timbro** è un `AssemblyMetadata` del suo csproj: senza, la barra
direbbe ancora 1.8.1) e i quattro di `wwwroot` che viaggiano insieme.

⚠️ **Due cose le ha corrette la prova sul pacchetto, non i test**:

1. **«fra ~0 min»** nella riga del giro. Sul pacchetto è comparso subito, e in produzione è il caso
   **normale**: il processo riparte spesso, e appena riparte l'ultimo giro è già più vecchio di un quarto
   d'ora. Zero minuti ora si dice a parole.
2. Il **plurale** e lo **spazio mangiato da Razor** erano già usciti dalla verifica live di §BM7: la stessa
   famiglia di difetti — quelli che un test non guarda perché guarda i numeri, non le frasi.

⚠️ **E i `packages.lock.json`**: i miei build ci avevano aggiunto sezioni RID che nessun cambio di
dipendenze giustificava. La CI restora in **locked mode** e si sarebbe fermata lì. Rimessi identici a
1.8.1 — cioè allo stato che la CI ha già accettato.

✅ **Caricato dal committente la notte del 4 settembre**, e **verificato da fuori due giri su due**: circuito
