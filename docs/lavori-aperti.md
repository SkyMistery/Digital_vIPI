...
### 🟢 AA1 — S4: l'ATZ per le torri senza poligono

Passo automatico **fra il ripiego del sectorfile e il cerchio da 5 NM**, in
`AirportSectorImportHostedService`. Fonte **secondaria**, come chiesto: se domani IVAO manda una shape vera,
IVAO vince. **42 ATZ su 46** portano l'ICAO nel nome (`ATZ CROTONE LIBC`); i quattro che non ce l'hanno sono
tre MATZ e un campo di airwork, che si agganciano a mano.

### 🟢 AA2 — S5: la pagina pubblica degli spazi aerei

`/services/airspace`, accanto al convertitore. Settori nostri + volumi del file, accendibili **per
famiglia**. ⚠️ I 144 892 punti al browser non ci vanno: le sole famiglie utilizzabili sono **362 volumi,
~620 KB**, e si mandano una famiglia per volta. ⚠️ L'attribuzione della fonte va **dove il dato si vede**,
qui e nelle mappe AoR che mostrano un confine agganciato.

### 🟢 AA3 — S6: il convertitore prende dal catalogo

Una sorgente in più fra le tredici che `/services/coordinates` già legge: «prendi un'area dal catalogo
spazi aerei» invece di ricaricare un file.

### 🟢 AA4 — S7: il rapporto radioassistenze e campi

**Solo segnalazione, nessuna scrittura** (decisione del committente): che cosa il file dice e l'anagrafica
no, e viceversa. Le correzioni si fanno nel **sectorfile** e poi si reimporta. Il file ha 78 VOR e 37 NDB
con **codice, tipo, frequenza, canale, portata, declinazione e posizione** — `ALGHERO` sono tre impianti
distinti (`AHO` TACAN, `AEA` DVOR-DME, `ALG` VORTAC), cioè l'identità che l'anagrafica già usa.
