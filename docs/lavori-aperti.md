...
**Aggiornato:** 4 settembre 2026, notte — 📦 **PACCHETTO 1.9.0 PRONTO** (`vipi-1.9.0-solo-file-cambiati.zip`, sha256 `3045b044acf77bdd2deefd2c1ed911e2d2a2ec6cce3995cfffede0c9143d73ee`, **4,50 MB**, **15 file**). Timbro **`1.9.0 · f75e2e0`**. ⚠️ **MINOR e non PATCH, per il CONTENUTO**: §BM porta una **sezione nuova** al Registro, un **tasto nuovo** nell'editor e due righe che prima non c'erano — una PATCH è «solo correzioni». 🟢 **Nessuna migrazione**: si consegna da sola via FTP anche dentro la finestra cieca. ⚠️ **C'è `wwwroot`**, ed è l'unica trappola vera: `vipi-theme.css` è cambiato (una regola, `.tr-wait`), quindi il foglio, i suoi `.br`/`.gz` e **l'indice degli asset** viaggiano **insieme** (difetto del 24 agosto). ⚠️ **E ci sono le FRASI** (53 chiavi nuove sui due `.resx`), quindi entra il satellite inglese. Dentro ci sono **§BM** (a che punto è la traduzione) e **§BM-bis** (il riquadro «ricarica la pagina», che **non** era l'host). ⚠️ **Vipi.Domain.dll entra e non è MVID**: `TranslationSpendKind` ha un valore nuovo, e col Domain vecchio quella colonna — che è TEXT — si riempirebbe di «2». ⚠️ `Vipi.Hosting.dll` ha impronta diversa e resta **fuori**: nessuna sua sorgente toccata, e la sola firma pubblica cambiata (`RegistraSpesaAsync`) la chiamano soltanto due assiemi, **tutti e due dentro il pacchetto** — cercato in Host, Hosting, Assets e MySqlMigrations, nessun riferimento. **Provato sul pacchetto pubblicato**: dieci controlli verdi, e il file provato è **esattamente** quello che si spedisce (sei impronte confrontate, tutte uguali). Le 15 impronte **riverificate dentro lo zip**. Foglio: `deploy/atc-ivao/LEGGIMI-PACCHETTO-1.9.0.md`. 🔴 Resta da **caricare via FTP**.
...
### BM9 📦 Il pacchetto 1.9.0, e le due cose che la prova ha corretto
