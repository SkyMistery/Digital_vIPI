...
⚠️ **DUE RAMI CON LAVORO FUORI DA `main`**, e sono cose diverse:

- **`intro-di-pagina`** — quattordici commit, **spinto** (`origin/intro-di-pagina`), da `main`. §AC + §AD.
  **Nessuna migrazione**, ventiquattro file, suite e release verdi. Pronto da fondere quando si vuole.
- **`consegna-db-20260830`** — undici commit, **SOLO LOCALE, da spingere**. È il pacchetto della consegna a
  Ivao.It (timbro della versione, fogli d'aggiornamento FTP, il presidio delle migrazioni della finestra
  cieca). ⚠️ Non c'entra con l'intro: tocca `Directory.Build.props`, `deploy/` e due file di test, e i due
  rami **non si sovrappongono**.

`main` è **allineato con `origin/main`**. Il 30 agosto sono
