...
- **Note:** overlay runtime (online settori, meteo/pista) sempre live sopra il congelato. Caveat accettato: AIRAC misto in pagina (sezione Live di ciclo diverso). Residuo minore: gate delle **liste** pubbliche `Status==Published`→ha-release non ancora cambiato (dopo il backfill Published≈ha-release; nessuna incoerenza pratica).
