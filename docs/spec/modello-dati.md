...
> - **§9 (round 13–20)** è la parte corrente e **prevale** dove in conflitto con §3–§5: cataloghi `AccSector`/`AirportSector`, frequenza = attributo del settore, `Fir`→`Acc`, e (§9.12) fonte unica + gerarchia per callsign.
> Per lo stato del codice vedi `../../HANDOFF.md`; per la storia `../history/rounds.md`.

> 🔴 **DOVE STA LA VERITÀ, dal 7 settembre 2026.** La sorgente dello schema è
> **`VipiDbContext.OnModelCreating`**, non questo documento: è lì che ogni scelta è scritta accanto alla
> riga che la applica, e le migrazioni le genera EF da quella. Questo documento resta la carta del
> **perché** — i principi, i confini fra le entità, le decisioni e la loro storia — e va letto così.
>
> La corona è stata spostata perché il documento non riusciva a portarla, e si vedeva:
> - **§9.8** si dichiarava «la lista migrazioni autoritativa» e si fermava alla **85ª di 114** — mancavano
>   sottosistemi interi (archivio ATC mondiale, biblioteca allegati, catalogo spazi aerei, memoria di
>   traduzione…);
> - **16 entità su 59** non compaiono in nessuna sezione (`Attachment`, `AirspaceVolume`, `TranslationUnit`,
>   `MediaAsset`, `EditResourceLock`, …);
> - **§9.13** descriveva per esteso tre cose che nel codice non esistono più.
>
> Un elenco che si aggiorna a mano ricade sempre, e un elenco che si dichiara completo senza esserlo costa
> più di uno che non promette niente (revisione del 6 settembre 2026, R-011 · R-012 · R-013).
>
> **L'elenco vero delle migrazioni, quando serve, si chiede:**
> ```bash
> dotnet ef migrations list --project src/Vipi.Infrastructure --startup-project src/Vipi.Host
> # oppure, senza strumenti: ls src/Vipi.Infrastructure/Persistence/Migrations/*.cs
> ```
> Al 7 settembre 2026: **114** migrazioni SQLite/Postgres e **48** MySQL/MariaDB (assieme separato,
> `Vipi.Infrastructure.MySqlMigrations`).

> ℹ️ **Documento di design.** Schema EF Core implementato (vedi `Vipi.Domain/Entities` + migrazioni). **Aggiunte rispetto a questa spec:** entità `Transfer` (+enum `TransferPhase`, catena handler = array JSON) e `RoleOverride` (promozioni a mano; ⚠️ `EditGrant`, i permessi per-ACC, è stata **eliminata** il 28 agosto 2026 — vedi `docs/feature/2026-08-28-autorizzazioni-a-livelli.md`); campi **lock** su `Document` (`LockedByVid/At/Expires`); `RowVersion` su `ContentBlock`/`DocumentSection` (concorrenza ottimistica). Stato codice in `../../README.md`/`../../HANDOFF.md`.
...
| `EditGrant` | `Vid` → `UserId`, `GrantedByVid` → `GrantedByUserId` — ⚠️ entità **eliminata** il 28 agosto 2026 |
...
### 9.8 Migrazioni — le storie, non l'elenco

> ⚠️ **Questo NON è l'elenco delle migrazioni**, e fino al 7 settembre 2026 diceva di esserlo: si
> fermava all'85ª di 114. L'elenco si chiede a EF (vedi il riquadro in testa al documento). Quel che
> resta qui è la parte che un comando non sa dire — **perché** una migrazione è fatta così: il drop del
> figlio prima del padre, le due migrazioni con un comando in mezzo, il rename che lo scaffolding
> proponeva diverso nei due provider. Sono le pagine da leggere prima di scriverne una nuova.
...
- **Orfani → il legame al documento RESTA** (⚠️ corretto il 27 ago 2026: questa voce diceva il contrario, e il codice fa così **dal 25 agosto**). Quando un settore proiettato diventa orfano si limita a `IsActive=false`. La ragione per cui prima si azzeravano anche `DocumentId`/`IsPrimary`/`FeaturedRank` era vera — un settore che non esiste più non deve restare agganciato a un `Document` — ma il prezzo era più alto del rimedio: **la riga tornava al giro successivo, il legame no**, e il documento restava sganciato per sempre, con la pagina pubblica muta. Ora il legame resta e la casella degli impatti avvisa; a recidere è l'admin, dalla sezione «Orfani» della Struttura. Il motivo originale resta coperto: chi risolve un documento filtra su `IsActive`, e la rigenerazione riallinea solo gli attivi.
- **`Sector.ImportedAtUtc` = il timbro della RIGA DI CATALOGO**, non l'ora della proiezione (⚠️ dal 27 ago 2026, audit prestazioni §O/P6). Prima era `DateTime.UtcNow`, e faceva due danni: la proiezione gira a **ogni avvio**, quindi EF marcava come modificati **tutti** i settori (312 UPDATE per boot, misurati); e soprattutto la **regola D8** delle eliminazioni — «la sorgente lo manda ancora?» — riceveva «sì, perché abbiamo riavviato». ⚠️ La correzione ovvia («timbra solo se qualcosa è cambiato») sarebbe stata **sbagliata**: `SogliaEliminazione.Consentita` risponde `timbro < penultimo`, quindi un settore **stabile** avrebbe smesso di essere timbrato e sarebbe diventato eliminabile mentre la sorgente lo manda ancora.
...
> 🛑 **[SUPERATO — non implementare da qui]** dal 7 settembre 2026. **Nessuna** delle tre cose
> descritte qui sotto esiste nel codice: `AppProfile`, `AppFrequencyLink` e `IAppProfileService` sono
> stati sostituiti da `Document` + `DocumentProfile`, e `VipiDbContext` lo dice («entità AppProfile
> rimosse»). Di questa sezione sopravvive la sola **rotta** (`/services/vsop/{acc}/apps/editor`).
> Resta come storia — le derivazioni descritte qui sono ancora il modello mentale giusto — non come
> istruzione (revisione del 6 settembre 2026, R-011).
...
- **Gating fail-safe**: bozza/release **gated** al livello (Editor); per non autorizzato, identità release non corrispondente (verifica `TargetType`/`TargetKey`) o URL forgiato → **degrada a pubblica** senza banner. Ciclo del banner dalla release (`ReleaseAiracCycle`), non da `now`.
...
- `ConditionAreaNegated : bool` (NOT NULL, default `false`) — la **polarità** dell'area (10 settembre 2026):
  ...
  delle varianti (capofila + eccezione), che la frase cumula già in AND.
