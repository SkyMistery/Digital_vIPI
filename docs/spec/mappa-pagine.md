...
| `/services/vsop/sectorfile` | `SectorfilePage.razor` | **Coerenza col sectorfile**: dove i cataloghi IVAO e il sectorfile Aurora non dicono la stessa cosa — frequenze, TA, designatori di pista, soglie. ⚠️ **Non confronta**: legge la fotografia che il giro delle 24 ore ha già preso (`ISectorfileComparisonReport`), più il tasto «confronta adesso» che rifà il giro **con lo stesso codice**. Non è un servizio ma una **scorciatoia**: nell'hub la scheda porta `shortcut`. Gli stessi rilievi stanno anche in Diagnostica, sotto il chip dell'area `Sectorfile` — una verità, due porte, e questa è quella che l'AOD può aprire. Carta `design/piano-coerenza-sectorfile.md` | staff di divisione |
