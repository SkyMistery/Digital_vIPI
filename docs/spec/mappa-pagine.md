...
> ⚠️ **Riallineata al codice il 13 settembre 2026** (T-057): mancavano 18 rotte, e la colonna Accesso
> diceva cancelli che il codice non aveva. I livelli sono i cinque di `VipiRole` (User · IvaoStaff ·
> DivisionStaff · Editor · Admin); «tutti» = anche anonimo. Il cancello vero sta nei servizi: questa colonna
> dice che cosa **disegna** la pagina. Per le voci di amministrazione la fonte è `AdminNav.razor` (`Minimo`).
...
| `/services/vsop/{acc}/vloa` | `VloaListPage.razor` | Elenco vLOA della ACC (no `?acc`); con `?acc=YYYY` mostra il **documento** della coppia acc↔YYYY (una rotta che ramifica per query, come aeroporti). Chiave = **codice ACC vicino** (`VloaRow.NeighbourCode`), non più docId | tutti (edit: Editor) |
...
| `/services/vsop/screens` | `ScreensIndex.razor` | Indice schermate: attrezzo della verifica live, non pagina di prodotto | tutti (nessun cancello) |
| `/services/vsop/sectorfile` | `SectorfilePage.razor` | **Coerenza col sectorfile**: dove i cataloghi IVAO e il sectorfile Aurora non dicono la stessa cosa — frequenze, TA, designatori di pista, soglie. ⚠️ **Non confronta**: legge la fotografia che il giro delle 24 ore ha già preso (`ISectorfileComparisonReport`), più il tasto «confronta adesso» che rifà il giro **con lo stesso codice**. Non è un servizio ma una **scorciatoia**: nell'hub la scheda porta `shortcut`. Gli stessi rilievi stanno anche in Diagnostica, sotto il chip dell'area `Sectorfile` — una verità, due porte, e questa è quella che apre anche un chief d'ACC. ⚠️ Cancello **Editor** — un gradino più su delle altre due scorciatoie della sezione staff — perché i rilievi parlano del contenuto dei documenti. Carta `design/piano-coerenza-sectorfile.md` | Editor |
| `/services/vsop/release/{id}` | `ReleasePreviewPage.razor` | **Redirect** (compat): risolve la release e reindirizza al viewer tipizzato con `?as=rel:{id}`. Le anteprime sono rese dentro i viewer, non più qui | Editor (cancello nel servizio) |
| `/services/vsop/versions`, `/services/vsop/{acc}/versions` | `VersioniPage.razor` | **Hub documenti unificato** (ex `/vsop/editor` assorbito): elenco completo doc (vIPI ACC/APP/aeroporto/vLOA) + ricerca/filtri, «Apri editor» per riga, tasto «Nuovo documento», storico versioni + release AIRAC. Azioni hide/annulla-release dall'Editor in su; **elimina solo Admin** (T-012, 13-set-2026: vale per ogni bersaglio, cestino spento col motivo). `Apri editor` gated server-side | Editor |
...
| `/services/vsop/admin/neighbours` | `ConfinantiAdminPage.razor` | ACC confinanti e settori esteri | Editor |
| `/services/vsop/admin/glossary` · `/services/vsop/admin/translations` | `GlossarioPage.razor` | Fraseologia e traduzioni | Editor |
| `/services/vsop/admin/navaids` | `AdminNavaidsPage.razor` | Radioassistenze | Editor |
| `/services/vsop/admin/attachments` | `AdminAttachmentsPage.razor` | Allegati (biblioteca PDF) | Editor |
| `/services/vsop/admin/airspace` | `AdminAirspacePage.razor` | Spazi aerei (caricamento) | Editor |
| `/services/vsop/admin/diagnostics` | `DiagnosticaPage.razor` | Diagnostica | Admin |
...
- **Lock editing admin (2026-07):** le 4 pagine di struttura (`/services/vsop/admin/sector-structure`, `/services/vsop/admin/acc`,
  ...
  («Inizia/Fine modifica» + banner + «Forza sblocco», dall'Editor in su dal 29 agosto 2026). Read-only finché non si prende il lock; TTL 3min +
...
- **Anteprime (`?as=`, round 33):** i 4 viewer documentali (`AccVipiPage`, `AeroportoPage`, `AppnPage`,
  ...
  di una release. Bozza/release sono **gated** al livello (Editor); per un utente non autorizzato,
...
- **Sotto-sezioni:** stanno **fra i blocchi** del padre, in una fila sola (`CorpoDiSezione`, 15 settembre 2026, §A35):
  posizione in `DocumentSection.BodyPosition` (soglia sull'`Order` dei blocchi; null = vale il vecchio
  `BeforeParentBody`, prima o dopo), si sposta con le frecce, uguale nei viewer e nell'editor. I tre slot del §3g non ci sono più.
