...
| `/services/vsop/versions`, `/services/vsop/{acc}/versions` | `VersioniPage.razor` | **Hub documenti unificato** (ex `/vsop/editor` assorbito): elenco completo doc (vIPI ACC/APP/aeroporto/vLOA) + ricerca/filtri, «Apri editor» per riga, tasto «Nuovo documento», storico versioni + release AIRAC. Azioni hide/elimina/annulla-release dall'Editor in su. `Apri editor` gated server-side | Editor |
